...
public:
    ...
    void RelayTransaction(const Wtxid& wtxid) override EXCLUSIVE_LOCKS_REQUIRED(!m_peer_mutex, !m_inv_to_send_mutex);
...
void PeerManagerImpl::PushNodeVersion(CNode& pnode, const Peer& peer)
{
    ...
    MakeAndPushMessage(
        ...
        NetMsgType::VERSION,
        PROTOCOL_VERSION,
        uint64_t{peer.m_our_services},
        TicksSinceEpoch<std::chrono::seconds>(NodeClock::now()),
        // addr.nServices + CNetAddr::V1(your_addr) is the pre-version-31402 serialization of your_addr (without nTime)
        uint64_t{addr.nServices}, CNetAddr::V1(your_addr),
        // same, for a dummy address
        uint64_t{peer.m_our_services}, CNetAddr::V1(CService{}),
        pnode.GetLocalNonce(),
        strSubVersion,
        my_height,
        my_tx_relay);
...
void PeerManagerImpl::ReattemptInitialBroadcast(CScheduler& scheduler)
{
    ...
    for (const auto& txid : unbroadcast_txids) {
        ...
        if (tx != nullptr) {
            RelayTransaction(tx->GetWitnessHash());
        } else {
...
void PeerManagerImpl::RelayTransaction(const Wtxid& wtxid)
{
    {
        LOCK(m_inv_to_send_mutex);
