    ...
    return RPCHelpMan{
        ...
        [&](const RPCHelpMan& self, const JSONRPCRequest& request) -> UniValue {
            ...
            if (!forecast_result.first.empty()) {
                result.pushKV("low", ValueFromAmount(forecast_result.first.m_forecast_opt.m_l_priority_estimate.GetFeePerK()));
                result.pushKV("high", ValueFromAmount(forecast_result.first.m_forecast_opt.m_h_priority_estimate.GetFeePerK()));
                result.pushKV("forecaster", forecastTypeToString(forecast_result.first.m_forecast_opt.m_forecaster));
            } else {
