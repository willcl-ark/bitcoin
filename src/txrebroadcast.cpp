...
TxRebroadcastHandler::TxRebroadcastHandler(const CTxMemPool& mempool, const ChainstateManager& chainman)
    : m_mempool{mempool},
      m_chainman{chainman},
...
{
    Chainstate& chain_state = m_chainman.ActiveChainstate();
    if (chain_state.IsInitialBlockDownload() || !m_mempool.GetLoadTried()) return;
    ...
    auto start_time = GetTime<std::chrono::microseconds>();
    {
        ...
        current_fee_rate = BlockAssembler(chain_state, &m_mempool, options).MinTxFeeRate();
    }
