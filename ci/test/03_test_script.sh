...
if [ "$CI_OS_NAME" = "freebsd" ]; then
  # FreeBSD 15.1 ships ccache 3.7.12, which does not support --verbose.
  ccache --show-stats
else
  ccache --show-stats --verbose
fi
if [ "$CI_OS_NAME" = "freebsd" ]; then
  # FreeBSD 15.1 ships ccache 3.7.12, whose --print-stats output is not
  # machine-readable and cannot be consumed by the hit-rate parser below.
  ccache --print-stats
else
