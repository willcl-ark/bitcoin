...
class BumpFeeTest(BitcoinTestFramework):
    ...
    def test_invalid_parameters(self, rbf_node, peer_node, dest_address):
        ...
        assert_raises_rpc_error(
            -8, "Invalid parameter, output argument cannot be an empty array", rbf_node.bumpfee, rbfid, {"outputs": []}
        )
        assert_raises_rpc_error(
            -8,
        ...
        assert_raises_rpc_error(
            -8, "Change position is out of range", rbf_node.bumpfee, rbfid, {"original_change_index": 2}
        )
        ...
        assert_raises_rpc_error(
            ...
            rbf_node.bumpfee,
            rbfid,
            {"original_change_index": 2, "outputs": [{dest_address: 0.1}]},
        )
    ...
    def test_bump_back_to_yourself(self):
        ...
        assert_equal(len(bumped_tx["decoded"]["vin"]), 2)
        assert any(
            txout["value"] == out_amount - bumped["fee"] and txout["scriptPubKey"]["address"] == change_addr
            for txout in bumped_tx["decoded"]["vout"]
        )
        # Check that total out amount is still equal to the previously bumped tx
        assert_equal(
            bumped_tx["decoded"]["vout"][0]["value"] + bumped_tx["decoded"]["vout"][1]["value"] + bumped["fee"], 10
        )
        ...
        assert_equal(
            bumped_tx["decoded"]["vout"][0]["value"] + bumped_tx["decoded"]["vout"][1]["value"] + bumped["fee"], 15
        )
    ...
    def test_single_output(self):
        ...
        assert_raises_rpc_error(
            ...
            wallet.bumpfee,
            txid=tx["txid"],
            options={"fee_rate": 1100, "original_change_index": 0},
        )
...
def test_segwit_bumpfee_succeeds(self, rbf_node, dest_address):
    ...
    rbfraw = rbf_node.createrawtransaction(
        ...
        {dest_address: Decimal("0.0005"), rbf_node.getrawchangeaddress(): Decimal("0.0003")},
    )
...
def test_notmine_bumpfee(self, rbf_node, peer_node, dest_address):
    ...
    inputs = [
        {"txid": utxo["txid"], "vout": utxo["vout"], "address": utxo["address"], "sequence": MAX_BIP125_RBF_SEQUENCE}
        for utxo in utxos
    ]
    ...
    assert_raises_rpc_error(-4, "Transaction contains inputs that don't belong to this wallet", rbf_node.bumpfee, rbfid)
...
def test_bumpfee_with_abandoned_descendant_succeeds(self, rbf_node, rbf_node_address, dest_address):
    ...
    child_id = rbf_node.send(
        outputs={dest_address: 0.00020000}, options={"inputs": [{"txid": parent_id, "vout": 0}], "fee_rate": 2}
    )["txid"]
...
def test_small_output_with_feerate_succeeds(self, rbf_node, dest_address):
    ...
    assert [
        ...
        if txin["txid"] == original_txin["txid"] and txin["vout"] == original_txin["vout"]
    ]
...
def test_watchonly_psbt(self, peer_node, rbf_node, dest_address):
    ...
    reqs = [
        ...
        {"desc": priv_change_desc, "timestamp": 0, "range": [0, 0], "internal": True, "keypool": False},
    ]
    ...
    psbt = watcher.walletcreatefundedpsbt(
        [watcher.listunspent()[0]], {dest_address: 0.0005}, 0, {"fee_rate": 1, "add_inputs": False}, True
    )["psbt"]
...
def test_rebumping_not_replaceable(self, rbf_node, dest_address):
    ...
    assert_raises_rpc_error(
        -4, "Transaction is not BIP 125 replaceable", rbf_node.bumpfee, bumped["txid"], {"fee_rate": NORMAL}
    )
    self.clear_mempool()
...
def test_bumpfee_already_spent(self, rbf_node, dest_address):
    ...
    assert_raises_rpc_error(
        -1, f"{spent_input['txid']}:{spent_input['vout']} is already spent", rbf_node.bumpfee, txid, fee_rate=NORMAL
    )
...
def test_unconfirmed_not_spendable(self, rbf_node, rbf_node_address):
    ...
    assert_raises_rpc_error(
        -5, "Transaction not eligible for abandonment", lambda: rbf_node.abandontransaction(txid=bumpid)
    )
    ...
    assert_equal(
        sum(
            ...
            if t["txid"] == rbfid and t["address"] == rbf_node_address and t["spendable"]
        ),
        1,
    )
    self.clear_mempool()
...
def test_locked_wallet_fails(self, rbf_node, dest_address):
    ...
    rbf_node.walletlock()
    assert_raises_rpc_error(
        -13, "Please enter the wallet passphrase with walletpassphrase first.", rbf_node.bumpfee, rbfid
    )
    rbf_node.walletpassphrase(WALLET_PASSPHRASE, WALLET_PASSPHRASE_TIMEOUT)
    self.clear_mempool()
