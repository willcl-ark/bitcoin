    ...
    def run_test(self):
        ...
        if self.options.descriptors:
            ...
            result = w1.importmulti(
                [{"scriptPubKey": {"address": key_to_p2wpkh(pubkey)}, "timestamp": "now", "keys": [privkey]}]
            )
        ...
        # Encrypt the wallet
        assert_raises_rpc_error(
            -16, "Error: wallet does not contain private keys, nothing to encrypt.", w5.encryptwallet, "pass"
        )
        assert_raises_rpc_error(-4, "Error: This wallet has no available keys", w5.getnewaddress)
        assert_raises_rpc_error(-4, "Error: This wallet has no available keys", w5.getrawchangeaddress)
        ...
        self.nodes[0].createwallet(
            wallet_name="wblank", disable_private_keys=False, blank=True, passphrase="thisisapassphrase"
        )
        ...
        self.nodes[0].createwallet(
            wallet_name="w6", disable_private_keys=False, blank=False, passphrase="thisisapassphrase"
        )
        ...
        assert_equal(
            resp["warnings"],
            [EMPTY_PASSPHRASE_MSG] if self.options.descriptors else [EMPTY_PASSPHRASE_MSG, LEGACY_WALLET_MSG],
        )
