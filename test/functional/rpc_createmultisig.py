...
class RpcCreateMultiSigTest(BitcoinTestFramework):
    ...
    def check_addmultisigaddress_errors(self):
        ...
        assert_raises_rpc_error(
            -5, "no full public key for address", lambda: self.nodes[0].addmultisigaddress(nrequired=1, keys=addresses)
        )
        for a in addresses:
    ...
    def test_multisig_script_limit(self, wallet_multi):
        ...
        assert_raises_rpc_error(
            -8, "redeemScript exceeds size limit: 684 > 520", node1.createmultisig, 16, pubkeys, "legacy"
        )
        ...
        if wallet_multi is not None and not self.options.descriptors:
            assert_raises_rpc_error(
                -8,
                "redeemScript exceeds size limit: 684 > 520",
                wallet_multi.addmultisigaddress,
                16,
                pubkeys,
                "",
                "legacy",
            ...
            assert_raises_rpc_error(
                -4,
                "Unsupported multisig script size for legacy wallet. Upgrade to descriptors to overcome this limitation for p2sh-segwit or bech32 scripts",
                wallet_multi.addmultisigaddress,
                16,
                pubkeys,
                "",
    ...
    def do_multisig(self, nkeys, nsigs, output_type, wallet_multi):
        ...
        self.log.info(
            ...
            % (nsigs, nkeys, output_type, txinfo["size"], txinfo["vsize"], txinfo["weight"])
        )
    ...
    def test_sortedmulti_descriptors_bip67(self):
        ...
        with open(
            os.path.join(os.path.dirname(os.path.realpath(__file__)), "data/rpc_bip67.json"), encoding="utf-8"
        ) as f:
            vectors = json.load(f)
