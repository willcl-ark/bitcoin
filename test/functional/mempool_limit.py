...
class MempoolLimitTest(BitcoinTestFramework):
    ...
    def test_rbf_carveout_disallowed(self):
        ...
        tx_B = self.wallet.create_self_transfer(
            fee=tx_A["fee"] * 4, target_weight=A_weight, utxo_to_spend=rbf_utxo, confirmed_only=True
        )
    ...
    def test_mid_package_eviction(self):
        ...
        mempool_evicted_tx = self.wallet.send_self_transfer(
            from_node=node, fee_rate=mempoolmin_feerate, target_weight=evicted_weight, confirmed_only=True
        )
        ...
        for i in range(num_big_parents):
            parent = self.wallet.create_self_transfer(
                fee_rate=parent_feerate, target_weight=parent_weight, confirmed_only=True
            )
    ...
    def run_test(self):
        ...
        assert_greater_than(
            target_weight_each * 2, node.getmempoolinfo()["maxmempool"] - node.getmempoolinfo()["bytes"]
        )
        ...
        tx_parent_just_below = miniwallet.create_self_transfer(
            fee_rate=parent_feerate, target_weight=target_weight_each
        )
        tx_child_just_above = miniwallet.create_self_transfer(
            utxo_to_spend=tx_parent_just_below["new_utxo"], fee_rate=child_feerate, target_weight=target_weight_each
        )
