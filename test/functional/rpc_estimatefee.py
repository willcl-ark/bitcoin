...
class EstimateFeeTest(BitcoinTestFramework):
    ...
    def run_test(self):
        ...
        assert_raises_rpc_error(
            -3, "JSON value of type string is not of expected type number", self.nodes[0].estimatesmartfee, "foo"
        )
        assert_raises_rpc_error(
            -3, "JSON value of type string is not of expected type number", self.nodes[0].estimaterawfee, "foo"
        )

        # wrong type for estimatesmartfee(estimate_mode)
        assert_raises_rpc_error(
            -3, "JSON value of type number is not of expected type string", self.nodes[0].estimatesmartfee, 1, 1
        )
        assert_raises_rpc_error(
            -8,
        ...
        assert_raises_rpc_error(
            -3, "JSON value of type string is not of expected type number", self.nodes[0].estimaterawfee, 1, "foo"
        )
        ...
        assert_raises_rpc_error(
            -8, "Invalid conf_target, must be between 1 and 1008", self.nodes[0].estimaterawfee, 1009
        )
