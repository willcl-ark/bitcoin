...
class ListSinceBlockTest(BitcoinTestFramework):
    ...
    def test_no_blockhash(self):
        ...
        assert_equal(self.nodes[0].listsinceblock(), {"lastblock": blockhash, "removed": [], "transactions": txs})
        assert_equal(self.nodes[0].listsinceblock(""), {"lastblock": blockhash, "removed": [], "transactions": txs})
    ...
    def test_targetconfirmations(self):
        ...
        assert_equal(
            self.nodes[0].getblockhash(0), self.nodes[0].listsinceblock(blockhash, blockheight + 1)["lastblock"]
        )
        assert_equal(
            self.nodes[0].getblockhash(0), self.nodes[0].listsinceblock(blockhash, blockheight + 1000)["lastblock"]
        )
        assert_raises_rpc_error(-8, "Invalid parameter", self.nodes[0].listsinceblock, blockhash, 0)
    ...
    def test_cant_read_block(self):
        ...
        assert_raises_rpc_error(
            -32603, "Can't read block from disk", self.nodes[0].listsinceblock, nodes1_last_blockhash
        )
    ...
    def test_double_spend(self):
        ...
        txid1 = self.nodes[1].sendrawtransaction(
            self.nodes[1].signrawtransactionwithwallet(self.nodes[1].createrawtransaction(utxo_dicts, recipient_dict))[
                "hex"
            ]
        )
        ...
        self.nodes[2].sendrawtransaction(
            self.nodes[2].signrawtransactionwithwallet(self.nodes[2].createrawtransaction(utxo_dicts, recipient_dict2))[
                "hex"
            ]
        )
    ...
    def double_spends_filtered(self):
        ...
        tx_input = dict(sequence=MAX_BIP125_RBF_SEQUENCE, **next(u for u in spending_node.listunspent()))
        ...
        block_hash = spending_node.listsinceblock(spending_node.getblockhash(spending_node.getblockcount()))
    ...
    def test_op_return(self):
        ...
        op_ret_tx = [
            tx for tx in self.nodes[2].listsinceblock(blockhash=block_hash)["transactions"] if tx["txid"] == tx_id
        ][0]
