...
class InvalidLocatorTest(BitcoinTestFramework):
    ...
    def run_test(self):
        ...
        for msg in [msg_getheaders(), msg_getblocks()]:
            ...
            msg.locator.vHave = [
                int(node.getblockhash(i - 1), 16) for i in range(block_count, block_count - (MAX_LOCATOR_SZ + 1), -1)
            ]
            ...
            msg.locator.vHave = [
                int(node.getblockhash(i - 1), 16) for i in range(block_count, block_count - (MAX_LOCATOR_SZ), -1)
            ]
