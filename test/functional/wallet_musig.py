            ...
            for placeholder in PLACEHOLDER_RE.findall(musig):
                ...
                for priv_desc in wallet.listdescriptors(True)["descriptors"]:
                    ...
                    if not desc.startswith("tr("):
                        continue
                    privkey_match = PRIVKEY_RE.search(desc)
                    assert privkey_match is not None
                ...
                for pub_desc in wallet.listdescriptors()["descriptors"]:
                    ...
                    if not desc.startswith("tr("):
                        continue
                    pubkey_match = PUBKEY_RE.search(desc)
                    assert pubkey_match is not None
                    ...
                    origin_path_match = ORIGIN_PATH_RE.search(pubkey)
                    assert origin_path_match is not None
                    privkey += origin_path_match.group(1)
                    break
