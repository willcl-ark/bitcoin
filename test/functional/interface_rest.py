...
class RESTTest(BitcoinTestFramework):
    ...
    def run_test(self):
        ...
        txid = self.wallet.send_to(
            from_node=self.nodes[0], scriptPubKey=getnewdestination()[1], amount=int(0.1 * COIN)
        )["txid"]
        self.sync_all()
        ...
        assert_greater_than_or_equal(int(hex_response.getheader("content-length")), json_obj["size"] * 2)
        ...
        self.test_rest_request(
            "/getutxos/checkmempool", http_method="POST", req_type=ReqType.JSON, status=400, ret_type=RetType.OBJ
        )
        ...
        self.test_rest_request(
            f"/getutxos/checkmempool/{long_uri}", http_method="POST", status=400, ret_type=RetType.OBJ
        )
        ...
        response_header = self.test_rest_request(
            f"/headers/{bb_hash}", req_type=ReqType.BIN, ret_type=RetType.OBJ, query_params={"count": 1}
        )
        ...
        response_header_hex = self.test_rest_request(
            f"/headers/{bb_hash}", req_type=ReqType.HEX, ret_type=RetType.OBJ, query_params={"count": 1}
        )
        ...
        resp_hex = self.test_rest_request(
            f"/blockhashbyheight/{block_json_obj['height']}", req_type=ReqType.HEX, ret_type=RetType.OBJ
        )
        assert_equal(resp_hex.read().decode("utf-8").rstrip(), bb_hash)
        resp_bytes = self.test_rest_request(
            f"/blockhashbyheight/{block_json_obj['height']}", req_type=ReqType.BIN, ret_type=RetType.BYTES
        )
        ...
        resp = self.test_rest_request(
            f"/blockfilterheaders/{INVALID_PARAM}/{bb_hash}", ret_type=RetType.OBJ, status=400
        )
        ...
        for num in ["5a", "-5", "0", "2001", "99999999999999999999999999999999999"]:
            assert_equal(
                ...
                self.test_rest_request(
                    f"/headers/{bb_hash}", ret_type=RetType.BYTES, status=400, query_params={"count": num}
                ),
            )
        ...
        json_obj = self.test_rest_request(
            "/mempool/contents", query_params={"verbose": "true", "mempool_sequence": "false"}
        )
        assert_equal(json_obj, raw_mempool_verbose)
        ...
        json_obj = self.test_rest_request(
            "/mempool/contents", query_params={"verbose": "false", "mempool_sequence": "true"}
        )
        ...
        resp = self.test_rest_request(
            "/mempool/contents", ret_type=RetType.OBJ, status=400, query_params={"verbose": "TRUE"}
        )
        assert_equal(
            resp.read().decode("utf-8").strip(), 'The "verbose" query parameter must be either "true" or "false".'
        )
        ...
        non_coinbase_txs = {tx["txid"] for tx in json_obj["tx"] if "coinbase" not in tx["vin"][0]}
        ...
        assert_equal(
            self.test_rest_request(f"/blockfilterheaders/basic/{bb_hash}", query_params={"count": 1}),
