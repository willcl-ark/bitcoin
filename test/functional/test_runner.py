...
def dump_ctest_inventory(config, *, extended):
    ...
    tests = []
    test_names = set()
    for portseed, test in enumerate(test_list, start=2):
        test_argv = test.split()
        ...
        labels = ["functional"]
        if extended and test_argv[0] in EXTENDED_SCRIPTS:
    ...
    print(json.dumps({"tests": tests}, sort_keys=True))
...
def main():
    ...
    parser.add_argument('--configfile', help=argparse.SUPPRESS)
    ...
    configfile = args.configfile or os.path.abspath(os.path.dirname(__file__)) + "/../config.ini"
