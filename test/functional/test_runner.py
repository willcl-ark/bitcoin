...
BASE_SCRIPTS = [
    # Organised into rough runtime buckets.
    # Longest test should go first, to favor running tests in parallel
    # Tests that take 2-5 minutes
    [
        "feature_fee_estimation.py",
        "feature_taproot.py",
        "feature_block.py",
    ],
    # Tests that take 1-2 minutes
    [
    ...
    # Tests that take 30 seconds to 1 minute
    [
    ...
    # Tests that take less than 30 seconds
    [
...
FLAT_BASE_SCRIPTS = [script for bucket in BASE_SCRIPTS for script in bucket]

# Place EXTENDED_SCRIPTS first since it has the 3 longest running tests
ALL_SCRIPTS = EXTENDED_SCRIPTS + FLAT_BASE_SCRIPTS
...
def validate_test_buckets(test_results, base_scripts):
    bucket_thresholds = [300, 120, 60, 30]
    misplaced_tests = []
    ...
    for test_result in test_results:
        ...
        current_bucket = None
        for i, bucket in enumerate(base_scripts):
            if test_name in bucket:
                current_bucket = i
                break

        # Skip if the test is not in BASE_SCRIPTS
        if current_bucket is None:
            continue

        if test_time > bucket_thresholds[current_bucket]:
            correct_bucket = next((i for i, threshold in enumerate(bucket_thresholds) if test_time <= threshold), len(bucket_thresholds))

            if correct_bucket != current_bucket:
                misplaced_tests.append((test_name, test_time, bucket_thresholds[current_bucket]))

    if misplaced_tests:
        logging.warning("\nThe following tests may be in the wrong time-bucket based on their elapsed time in this run:")
        for test, test_time, threshold in misplaced_tests:
            logging.warning(f"  {test} - took {test_time}s vs {threshold}s")
    else:
        logging.debug("\nAll tests are in the correct runtime buckets.")
...
def run_tests(*, test_list, src_dir, build_dir, tmpdir, jobs=1, enable_coverage=False, args=None, combined_logs_len=0, failfast=False, use_term_control, results_filepath=None):
    ...
    validate_test_buckets(test_results, BASE_SCRIPTS)
