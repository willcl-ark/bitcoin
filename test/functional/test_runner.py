...
def main():
    ...
    configfile = Path(__file__).resolve().parent / "../config.ini"
    ...
    tmpdir = Path(args.tmpdirprefix) / f"test_runner_₿_🏃_{datetime.datetime.now().strftime('%Y%m%d_%H%M%S')}"
    ...
    if args.help:
        ...
        parser.print_help()
        subprocess.check_call([sys.executable, Path(config["environment"]["SRCDIR"]) / 'test' / 'functional' / test_list[0].split()[0], '-h'])
        sys.exit(0)

    check_script_list(src_dir=Path(config["environment"]["SRCDIR"]), fail_on_warn=args.ci)
    ...
    run_tests(
        test_list=test_list,
        src_dir=Path(config["environment"]["SRCDIR"]),
        build_dir=Path(config["environment"]["BUILDDIR"]),
        tmpdir=tmpdir,
        jobs=args.jobs,
...
def run_tests(*, test_list, src_dir: Path, build_dir: Path, tmpdir: Path, jobs=1, enable_coverage=False, args=None, combined_logs_len=0, failfast=False, use_term_control):
    args = args or []
    ...
    if cache_dir.is_dir():
        print(f"{BOLD[1]}WARNING!{BOLD[0]} There is a cache directory here: {cache_dir}. If tests fail unexpectedly, try deleting the cache directory.")
    ...
    if len(test_list) > 1 and jobs > 1:
        # Populate cache
        try:
            subprocess.check_output([sys.executable, str(tests_dir / "create_cache.py")] + flags + [f"--tmpdir={tmpdir / 'cache'}"])
    ...
    while i < test_count:
        ...
        for test_result, testdir, stdout, stderr, skip_reason in job_queue.get_next():
            ...
            if test_result.status == "Passed":
                ...
                print(BOLD[1] + 'stdout:\n' + BOLD[0] + stdout + '\n')
                print(BOLD[1] + 'stderr:\n' + BOLD[0] + stderr + '\n')
                if combined_logs_len and testdir.isdir():
                    # Print the final `combinedlogslen` lines of the combined logs
                    print('{}Combine the logs and print the last {} lines ...{}'.format(BOLD[1], combined_logs_len, BOLD[0]))
                    print('\n============')
                    print('{}Combined log for {}:{}'.format(BOLD[1], testdir, BOLD[0]))
                    print('============\n')
                    combined_logs_args = [sys.executable, (tests_dir / 'combine_logs.py'), testdir]
                    if BOLD[0]:
...
def check_script_list(*, src_dir, fail_on_warn):
    ...
    python_files = set([test_file.name for test_file in script_dir.iterdir() if str(test_file).endswith(".py")])
...
class RPCCoverage():
    ...
    def _get_uncovered_rpc_commands(self):
        ...
        coverage_ref_filename = self.dir / reference_filename
        ...
        if not coverage_ref_filename.is_file():
