...
def main():
    ...
    parser.add_argument(
        "--coverage", action="store_true", help="generate a basic coverage report for the RPC interface"
    )
    parser.add_argument(
        "--ci",
        action="store_true",
    ...
    parser.add_argument(
        "--extended", action="store_true", help="run the extended test suite in addition to the basic tests"
    )
    parser.add_argument("--help", "-h", "-?", action="store_true", help="print help text and exit")
    parser.add_argument(
        "--jobs", "-j", type=int, default=4, help="how many test scripts to run in parallel. Default=4."
    )
    parser.add_argument(
        "--keepcache",
        "-k",
    ...
    if args.exclude:
        ...
        def print_warning_missing_test(test_name):
            print(
                "{}WARNING!{} Test '{}' not found in current test list. Check the --exclude list.".format(
                    BOLD[1], BOLD[0], test_name
                )
            )
            if fail_on_warn:
                sys.exit(1)
    ...
    if args.help:
        ...
        subprocess.check_call(
            [
                sys.executable,
                os.path.join(config["environment"]["SRCDIR"], "test", "functional", test_list[0].split()[0]),
                "-h",
            ]
        )
        sys.exit(0)
    ...
    if args.nocleanup:
        if shutil.disk_usage(tmpdir).free < MIN_NO_CLEANUP_SPACE:
            print(
                f"{BOLD[1]}WARNING!{BOLD[0]} There may be insufficient free space in {tmpdir} to run the functional test suite with --nocleanup. "
...
def run_tests(
    *,
    ...
    if shutil.disk_usage(tmpdir).free < min_space:
        print(
            f"{BOLD[1]}WARNING!{BOLD[0]} There may be insufficient free space in {tmpdir} to run the Bitcoin functional test suite. "
        ...
        for test_result, testdir, stdout, stderr, skip_reason in job_queue.get_next():
            ...
            if test_result.status == "Passed":
                ...
                if "[Errno 28] No space left on device" in stdout:
                    sys.exit(
                        f"Early exiting after test failure due to insufficient free space in {tmpdir}\n"
        ...
        while True:
            ...
            for job in self.jobs:
                ...
                if proc.poll() is not None:
                    ...
                    dot_count = 0
                    ret.append(
                        (TestResult(name, status, int(time.time() - start_time)), testdir, stdout, stderr, skip_reason)
                    )
            if ret:
                return ret
            if self.use_term_control:
