...
def default_controlblock(ctx):
    ...
    return (
        bytes([get(ctx, "leafversion") + get(ctx, "negflag")]) + get(ctx, "pubkey_internal") + get(ctx, "merklebranch")
    )
...
VALID_SIGHASHES_TAPROOT_SINGLE = [SIGHASH_SINGLE, SIGHASH_ANYONECANPAY + SIGHASH_SINGLE]
...
def spenders_taproot_active():
    ...
    for annex in [None, lambda _: bytes([ANNEX_TAG]) + random.randbytes(random.randrange(0, 250))]:
        ...
        for hashtype in VALID_SIGHASHES_TAPROOT:
            ...
            tap = taproot_construct(pubs[0])
            add_spender(
                spenders, "sighash/purepk", tap=tap, key=secs[0], **common, **SIGHASH_BITFLIP, **ERR_SCHNORR_SIG
            )
            ...
            if hashtype in VALID_SIGHASHES_TAPROOT_SINGLE:
                add_spender(
                    ...
                    annex=annex,
                    standard=no_annex,
                    hashtype_actual=random.choice(VALID_SIGHASHES_TAPROOT_NO_SINGLE),
                    failure={"hashtype_actual": hashtype},
                    **ERR_SCHNORR_SIG_HASHTYPE,
        ...
        scripts = [
            (
                "pk_codesep",
                CScript(random_checksig_style(pubs[1]) + bytes([OP_CODESEPARATOR])),
            ),  # codesep after checksig
            (
                "codesep_pk",
                CScript(bytes([OP_CODESEPARATOR]) + random_checksig_style(pubs[1])),
            ),  # codesep before checksig
            (
                "branched_codesep",
                CScript(
                    [
                        random.randbytes(random.randrange(2, 511)),
                        OP_DROP,
                        OP_IF,
                        OP_CODESEPARATOR,
                        pubs[0],
    ...
    add_spender(
        ...
        **common,
        **SINGLE_SIG,
        failure={"sighash": override(default_sighash, script_taproot=tap.leaves["codesep_pk"].script)},
        **ERR_SCHNORR_SIG,
    )
    add_spender(
        spenders,
        ...
        **SINGLE_SIG,
        failure={
            "sighash": override(
                default_sighash,
                leafversion=random.choice([x & 0xFE for x in range(0x100) if x & 0xFE != LEAF_VERSION_TAPSCRIPT]),
            )
        },
        **ERR_SCHNORR_SIG,
    )
    ...
    for hashtype in [SIGHASH_DEFAULT, random.choice(VALID_SIGHASHES_TAPROOT)]:
        ...
        add_spender(
            ...
            **SINGLE_SIG,
            sign=b"",
            failure={"sign": lambda _: random.randbytes(random.randrange(1, 63))},
            **ERR_SCHNORR_SIG_SIZE,
        )
        add_spender(
            spenders,
            ...
            **SINGLE_SIG,
            sign=b"",
            failure={"sign": lambda _: random.randbytes(random.randrange(66, 100))},
            **ERR_SCHNORR_SIG_SIZE,
        )
        ...
        add_spender(
            ...
            hashtype=hashtype,
            **SIG_ADD_ZERO,
            **(ERR_SCHNORR_SIG_HASHTYPE if hashtype == SIGHASH_DEFAULT else ERR_SCHNORR_SIG_SIZE),
        )
        add_spender(
            spenders,
            "siglen/padzero_csv",
        ...
        add_spender(
            ...
            hashtype=hashtype,
            **SIG_POP_BYTE,
            **(ERR_SCHNORR_SIG_SIZE if hashtype == SIGHASH_DEFAULT else ERR_SCHNORR_SIG),
        )
        add_spender(
            spenders,
            "siglen/popbyte_csv",
        ...
        add_spender(
            ...
            **SINGLE_SIG,
            sign=b"",
            failure={"sign": default_sign, "sighash": bitflipper(default_sighash)},
            **ERR_SCHNORR_SIG,
        )
        add_spender(
            spenders,
    ...
    scripts = [
        ...
        [("129deep", CScript([pubs[0], OP_CHECKSIG])), random.choice(PARTNER_MERKLE_FN)],
    ]
    ...
    add_spender(
        ...
        **ERR_TAPROOT_WRONG_CONTROL_SIZE,
    )
    ...
    add_spender(
        ...
        **SINGLE_SIG,
        key=secs[0],
        failure={"witness": []},
        **ERR_WITNESS_PROGRAM_WITNESS_EMPTY,
    )
    ...
    add_spender(
        ...
        **SINGLE_SIG,
        key=secs[0],
        failure={"controlblock": lambda ctx: default_controlblock(ctx) + random.randbytes(random.randrange(1, 32))},
        **ERR_TAPROOT_WRONG_CONTROL_SIZE,
    )
    ...
    add_spender(
        ...
        **SINGLE_SIG,
        key=secs[0],
        failure={"controlblock": lambda ctx: default_merklebranch(ctx)[0 : random.randrange(1, 32)]},
        **ERR_TAPROOT_WRONG_CONTROL_SIZE,
    )
    ...
    csa_high_val = (
        random.randrange(17, 100) if random.getrandbits(1) else random.randrange(-100, -1)
    )  # Outside OP_n range
    ...
    add_spender(
        ...
        **common,
        standard=False,
        inputs=[getter("sign"), random.randbytes(MAX_SCRIPT_ELEMENT_SIZE)],
        failure={"inputs": [getter("sign"), random.randbytes(MAX_SCRIPT_ELEMENT_SIZE + 1)]},
        **ERR_PUSH_SIZE,
    )
    ...
    add_spender(
        ...
        **common,
        standard=False,
        inputs=[getter("sign"), random.randbytes(81)],
    )
    ...
    add_spender(
        spenders, "tapscript/1000stack", leaf="t21", **SINGLE_SIG, **common, failure={"leaf": "t22"}, **ERR_STACK_SIZE
    )
    ...
    add_spender(
        ...
        **common,
        ...
        failure={"leaf": "t24", "inputs": [getter("sign")] + [b"" for _ in range(1000)]},
        **ERR_STACK_SIZE,
    )
    # Test that pushing a MAX_SCRIPT_ELEMENT_SIZE byte stack element is valid, but one longer is not.
    add_spender(
        spenders, "tapscript/pushmaxlimit", leaf="t25", **common, **SINGLE_SIG, failure={"leaf": "t26"}, **ERR_PUSH_SIZE
    )
    ...
    SIGOPS_RATIO_SCRIPTS = [
        ...
        lambda n, pk: (
            CScript(
                [random.randbytes(220), OP_2DROP, pk, OP_1, OP_NOTIF, OP_2DUP, OP_CHECKSIG, OP_VERIFY, OP_ENDIF]
                + [OP_2DUP, OP_CHECKSIGVERIFY] * n
        ...
        lambda n, pk: (
            CScript(
                [OP_DROP, OP_0, OP_10, pk, OP_CHECKSIGADD, OP_10, OP_EQUALVERIFY, pk]
                + [OP_2DUP, OP_16, OP_SWAP, OP_CHECKSIGADD, b"\x11", OP_EQUALVERIFY] * n
                + [OP_CHECKSIG]
            ),
            n + 1,
        ),
            ...
            for pubkey in [pubs[1], random.randbytes(random.choice([x for x in range(2, 81) if x != 32]))]:
                for fn_num, fn in enumerate(SIGOPS_RATIO_SCRIPTS):
                    ...
                    add_spender(
                        ...
                        hashtype=hashtype,
                        key=secs[1],
                        inputs=[getter("sign"), random.randbytes(dummylen)],
                        standard=standard,
                        failure={"inputs": [getter("sign"), random.randbytes(dummylen - 1)]},
                        **ERR_TAPSCRIPT_VALIDATION_WEIGHT,
                    )
    ...
    tap = taproot_construct(
        pubs[0], [("leaf", CScript([pubs[1], OP_CHECKSIG, pubs[1], OP_CHECKSIGADD, OP_2, OP_EQUAL])), zero_fn]
    )
    add_spender(
        spenders, "case24765", tap=tap, leaf="leaf", inputs=[getter("sign"), getter("sign")], key=secs[1], no_fail=True
    )
...
def sample_spenders():
    ...
    add_spender(
        spenders,
        comment="tutorial/pushredux",
        tap=tap,
        leaf="2byte_push",
        inputs=[b"\x00"],
        failure={"leaf": "dummyleaf"},
        **ERR_EVAL_FALSE,
    )
    ...
    add_spender(
        spenders,
        comment="tutorial/nonminpush",
        tap=tap,
        leaf="nonstd_2byte_push",
        key=secs[0],
        standard=False,
        failure={"inputs": [getter("sign")]},
        **ERR_CLEANSTACK,
    )
...
def dump_json_test(tx, input_utxos, idx, success, failure):
    ...
    flags = (
        LEGACY_FLAGS
        if spender.comment.startswith("legacy/") or spender.comment.startswith("inactive/")
        else TAPROOT_FLAGS
    )
...
class TaprootTest(BitcoinTestFramework):
    ...
    def block_submit(
        self, node, txs, msg, err_msg, cb_pubkey=None, fees=0, sigops_weight=0, witness=False, accept=False
    ):
        ...
        extra_output_script = CScript(
            bytes([OP_CHECKSIG] * ((MAX_BLOCK_SIGOPS_WEIGHT - sigops_weight) // WITNESS_SCALE_FACTOR))
        )

        coinbase_tx = create_coinbase(
            self.lastblockheight + 1, pubkey=cb_pubkey, extra_output_script=extra_output_script, fees=fees
        )
        ...
        if err_msg is not None:
            assert block_response is not None and err_msg in block_response, (
                "Missing error message '%s' from block response '%s': %s"
                % (err_msg, "(None)" if block_response is None else block_response, msg)
            )
        if accept:
    ...
    def test_spenders(self, node, spenders, input_counts):
        ...
        while done < len(spenders):
            ...
            self.log.debug(
                ...
                % (count_this_tx, len(unspents), balance * 0.00000001, change_goal * 0.00000001)
            )
            ...
            for i in range(count_this_tx):
                utxodata = UTXOData(
                    outpoint=COutPoint(fund_tx.sha256, i), output=fund_tx.vout[i], spender=spenders[done]
                )
        ...
        while left:
            ...
            acceptable = [
                ...
                if n <= left and (left - n > max(input_counts) or (left - n) in [0] + input_counts)
            ]
            ...
            in_value = amount - fee
            tx.vin = [
                CTxIn(outpoint=utxo.outpoint, nSequence=random.randint(min_sequence, 0xFFFFFFFF))
            ...
            num_outputs = random.choice(
                range(1, 1 + min(4, 4 if first_mismatch_input is None else first_mismatch_input))
            )
            ...
            for fail_input in list(range(len(input_utxos))) + [None]:
                ...
                tx.rehash()
                msg = ",".join(
                    utxo.spender.comment + ("*" if n == fail_input else "") for n, utxo in enumerate(input_utxos)
                )
                if is_standard_tx:
    ...
    def gen_test_vectors(self):
        ...
        script_lists = [
            ...
            [
                ...
                [
                    ("1", CScript([pubs[55], OP_CHECKSIG]), LEAF_VERSION_TAPSCRIPT),
                    ("2", CScript([pubs[56], OP_CHECKSIG]), LEAF_VERSION_TAPSCRIPT),
                ],
            ],
            [
                ("0", CScript([pubs[57], OP_CHECKSIG]), LEAF_VERSION_TAPSCRIPT),
                [
                    ("1", CScript([pubs[58], OP_CHECKSIG]), LEAF_VERSION_TAPSCRIPT),
                    ("2", CScript([pubs[59], OP_CHECKSIG]), LEAF_VERSION_TAPSCRIPT),
                ],
            ],
        ]
        ...
        cblks = [
            {leaf: get({**DEFAULT_CONTEXT, "tap": taps[i], "leaf": leaf}, "controlblock") for leaf in taps[i].leaves}
            for i in range(7)
        ]
        ...
        for i in range(len(input_spks)):
            ...
            if ctx["mode"] == "taproot":
                ...
                intermediary["precomputedUsed"] = [
                    key for key in sorted(precomputed.keys()) if sigmsg.count(precomputed[key])
                ]
