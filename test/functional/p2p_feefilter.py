...
class FeeFilterTest(BitcoinTestFramework):
    ...
    def test_feefilter(self):
        ...
        txids = [
            miniwallet.send_self_transfer(fee_rate=Decimal("0.00000200"), from_node=node1)["wtxid"] for _ in range(3)
        ]
        ...
        txids = [
            miniwallet.send_self_transfer(fee_rate=Decimal("0.00000150"), from_node=node1)["wtxid"] for _ in range(3)
        ]
        conn.wait_for_invs_to_match(txids)
        conn.clear_invs()
        ...
        txids = [
            miniwallet.send_self_transfer(fee_rate=Decimal("0.00000100"), from_node=node1)["wtxid"] for _ in range(3)
        ]
        ...
        # as well.
        txids = [
            miniwallet.send_self_transfer(fee_rate=Decimal("0.00020000"), from_node=node0)["wtxid"] for _ in range(1)
        ]
        conn.wait_for_invs_to_match(txids)
        conn.clear_invs()
        ...
        txids = [
            miniwallet.send_self_transfer(fee_rate=Decimal("0.00020000"), from_node=node1)["wtxid"] for _ in range(3)
        ]
        conn.wait_for_invs_to_match(txids)
        conn.clear_invs()
