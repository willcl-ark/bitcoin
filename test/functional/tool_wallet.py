...
class ToolWalletTest(BitcoinTestFramework):
    ...
    def do_tool_createfromdump(self, wallet_name, dumpfile, file_format=None):
        ...
        assert (self.nodes[0].chain_path / "wallets" / wallet_name).is_dir()

        self.assert_tool_output("The dumpfile may contain private keys. To ensure the safety of your Bitcoin, do not share the dumpfile.\n", '-wallet={}'.format(wallet_name), '-dumpfile={}'.format(rt_dumppath), 'dump')

        rt_dump_data = self.read_dump(rt_dumppath)
        wallet_dat = self.nodes[0].chain_path / "wallets" / wallet_name / "wallet.dat"
    ...
    def test_invalid_tool_commands_and_args(self):
        ...
        locked_dir = self.nodes[0].chain_path / "wallets"
        ...
        path = self.nodes[0].chain_path / "wallets" / "nonexistent.dat"
    ...
    def test_dump_createfromdump(self):
        ...
        wallet_path = self.nodes[0].chain_path / "wallets" / "todump2"
        ...
        assert not (self.nodes[0].chain_path / "wallets" / "badload").is_dir()
    ...
    def run_test(self):
        self.wallet_path = os.path.join(self.nodes[0].chain_path, 'wallets', self.default_wallet_name, self.wallet_data_filename)
