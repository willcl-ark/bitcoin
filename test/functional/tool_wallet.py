...
class ToolWalletTest(BitcoinTestFramework):
    def add_options(self, parser):
        ...
        parser.add_argument(
            "--bdbro",
            action="store_true",
            help="Use the BerkeleyRO internal parser when dumping a Berkeley DB wallet file",
        )
        parser.add_argument(
            "--swap-bdb-endian",
            action="store_true",
            help="When making Legacy BDB wallets, always make then byte swapped internally",
        )
    ...
    def bitcoin_wallet_process(self, *args):
        ...
        return subprocess.Popen(
            [self.options.bitcoinwallet] + default_args + list(args),
    ...
    def do_tool_createfromdump(self, wallet_name, dumpfile, file_format=None):
        ...
        args = ["-wallet={}".format(wallet_name), "-dumpfile={}".format(dumppath)]
        ...
        if file_format is not None and file_format != dump_data["format"]:
            load_output += (
                'Warning: Dumpfile wallet format "{}" does not match command line specified format "{}".\n'.format(
    ...
    def test_invalid_tool_commands_and_args(self):
        ...
        self.assert_raises_tool_error(
            "Failed to load database path '{}'. Path does not exist.".format(path), "-wallet=nonexistent.dat", "info"
        )
    ...
    def test_dump_createfromdump(self):
        ...
        self.assert_raises_tool_error(
            "No dump file provided. To use dump, -dumpfile=<filename> must be provided.", "-wallet=todump", "dump"
        )
        ...
        self.assert_raises_tool_error(
            "Failed to create database path '{}'. Database already exists.".format(wallet_path),
            "-wallet=todump2",
            "-dumpfile={}".format(wallet_dump),
            "createfromdump",
        )
        ...
        self.assert_raises_tool_error(
            "Error: Dumpfile version is not supported. This version of bitcoin-wallet only supports version 1 dumpfiles. Got dumpfile with version 0",
            "-wallet=badload",
        ...
        self.assert_raises_tool_error(
            "Error: Dumpfile checksum does not match. Computed {}, expected {}".format(checksum, "1" * 64),
            "-wallet=bad",
        ...
        self.assert_raises_tool_error(
            "Error: Missing checksum", "-wallet=badload", "-dumpfile={}".format(bad_sum_wallet_dump), "createfromdump"
        )
        assert not (self.nodes[0].wallets_path / "badload").is_dir()
