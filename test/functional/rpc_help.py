...
class HelpRpcTest(BitcoinTestFramework):
    ...
    def test_client_conversion_table(self):
        ...
        for argname, convert in converts_by_argname.items():
            if all(convert) != any(convert):
                ...
                assert argname in ["dummy", "psbt"], "WARNING: conversion mismatch for argument named %s (%s)" % (
                    argname,
                    list(zip(all_methods_by_argname[argname], converts_by_argname[argname])),
                )
    ...
    def dump_help(self):
        ...
        os.mkdir(dump_dir)
        calls = [
            line.split(" ", 1)[0] for line in self.nodes[0].help().splitlines() if line and not line.startswith("==")
        ]
        for call in calls:
