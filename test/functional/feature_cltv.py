    ...
    def run_test(self):
        ...
        for i in range(5):
                ...
                [
                    {
                        ...
                        "reject-details": expected_cltv_reject_reason
                        + f", input 0 of {spendtx_txid} (wtxid {spendtx_wtxid}), spending {coin_txid}:{coin_vout}",
                    }
                ],
            ...
            with self.nodes[0].assert_debug_log(
                expected_msgs=[f"Block validation error: {expected_cltv_reject_reason}"]
            ):
                peer.send_and_ping(msg_block(block))
                assert_equal(int(self.nodes[0].getbestblockhash(), 16), tip)
                peer.sync_with_ping()
