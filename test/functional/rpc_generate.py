...
class RPCGenerateTest(BitcoinTestFramework):
    ...
    def test_generatetoaddress(self):
        ...
        assert_raises_rpc_error(
            -5, "Invalid address", self.generatetoaddress, self.nodes[0], 1, "3J98t1WpEZ73CNmQviecrnyiWrnqRhWNLy"
        )
    ...
    def test_generateblock(self):
        ...
        assert_raises_rpc_error(
            -5, "Transaction " + missing_txid + " not in mempool.", self.generateblock, node, address, [missing_txid]
        )

        self.log.info("Fail to generate block with invalid raw tx")
        invalid_raw_tx = "0000"
        assert_raises_rpc_error(
            -22, "Transaction decode failed for " + invalid_raw_tx, self.generateblock, node, address, [invalid_raw_tx]
        )
        ...
        assert_raises_rpc_error(
            -5, "Cannot derive script without private keys", self.generateblock, node, child_descriptor, []
        )
