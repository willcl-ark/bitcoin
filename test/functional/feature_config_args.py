...
class ConfArgsTest(BitcoinTestFramework):
    ...
    def test_negated_config(self):
        ...
        self.log.debug(
            "Verifying that disabling of the config file means garbage inside of it does "
            "not prevent the node from starting, and message about existing config file is logged"
        )
        ignored_file_message = [
            f'[InitConfig] Data directory "{self.nodes[0].datadir_path}" contains a "bitcoin.conf" file which is explicitly ignored using -noconf.'
        ]
    ...
    def test_log_buffer(self):
        with self.nodes[0].assert_debug_log(
            expected_msgs=["Warning: parsed potentially confusing double-negative -listen=0\n"]
        ):
    ...
    def test_connect_with_seednode(self):
        ...
        with self.nodes[0].assert_debug_log(expected_msgs=addcon_thread_started, unexpected_msgs=dnsseed_ignored):
        ...
        for connect_arg in ["-connect=0", "-noconnect"]:
            with self.nodes[0].assert_debug_log(expected_msgs=addcon_thread_started, unexpected_msgs=seednode_ignored):
    ...
    def test_acceptstalefeeestimates_arg_support(self):
        ...
        for chain, chain_name in {("main", ""), ("test", "testnet3"), ("signet", "signet"), ("testnet4", "testnet4")}:
            ...
            self.nodes[0].assert_start_raises_init_error(
                expected_msg=f"Error: acceptstalefeeestimates is not supported on {chain} chain."
            )
    ...
    def run_test(self):
        ...
        self.nodes[0].assert_start_raises_init_error(
            [f"-datadir={new_data_dir}"], f'Error: Specified data directory "{new_data_dir}" does not exist.'
        )
