...
class AssumeutxoTest(BitcoinTestFramework):
    ...
    def test_invalid_snapshot_scenarios(self, valid_snapshot_path):
        ...
        def expected_error(msg):
            assert_raises_rpc_error(
                -32603, f"Unable to load UTXO snapshot: Population failed: {msg}", node.loadtxoutset, bad_snapshot_path
            )
        ...
        for version in [0, 1, 3]:
            ...
            assert_raises_rpc_error(
                parsing_error_code,
                f"Unable to parse metadata: Version of snapshot {version} does not match any of the supported versions.",
                node.loadtxoutset,
                bad_snapshot_path,
            )
        ...
        for [magic, name, real] in invalid_magics:
            ...
            if real:
                assert_raises_rpc_error(
                    parsing_error_code,
                    f"Unable to parse metadata: The network of the snapshot ({name}) does not match the network of this node (regtest).",
                    node.loadtxoutset,
                    bad_snapshot_path,
                )
            else:
        ...
        for bad_block_hash in [bogus_block_hash, prev_block_hash]:
            with open(bad_snapshot_path, "wb") as f:
                f.write(
                    valid_snapshot_contents[:11] + bytes.fromhex(bad_block_hash)[::-1] + valid_snapshot_contents[43:]
                )
        ...
        for off in [-1, +1]:
            ...
            expected_error(
                msg="Bad snapshot - coins left over after deserializing 298 coins."
                if off == -1
        ...
        for content, offset, wrong_hash, custom_message in cases:
            ...
            msg = (
                ...
                else f"Bad snapshot content hash: expected d2b051ff5e8eef46520350776f4100dd710a63447a8e01d917e92e79751a63e2, got {wrong_hash}."
            )
            expected_error(msg)
    ...
    def test_sync_from_assumeutxo_node(self, snapshot):
        ...
        for block_num in range(1, miner.getblockcount() + 1):
            msg.headers.append(
                from_hex(CBlockHeader(), miner.getblockheader(miner.getblockhash(block_num), verbose=False))
            )
        ...
        self.wait_until(
            lambda: next(filter(lambda x: x["hash"] == headers_tip_hash, snapshot_node.getchaintips()), default_value)[
                "status"
            ]
            == "headers-only"
        )
        ...
        self.wait_until(
            lambda: next(filter(lambda x: x["hash"] == snapshot_block_hash, ibd_node.getchaintips()), default_value)[
                "status"
            ]
            == "headers-only"
        )
        ...
        self.wait_until(
            lambda: not try_rpc(
                -1, "Block not available (not fully downloaded)", snapshot_node.getblock, snapshot_block_hash
            )
        )
    ...
    def run_test(self):
        ...
        def check_dump_output(output):
            assert_equal(output["txoutset_hash"], "d2b051ff5e8eef46520350776f4100dd710a63447a8e01d917e92e79751a63e2")
        ...
        assert_equal(dump_output4["txoutset_hash"], "45ac2777b6ca96588210e2a4f14b602b41ec37b8b9370673048cc0af434a1ec8")
