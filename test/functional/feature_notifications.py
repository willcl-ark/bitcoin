...
class NotificationsTest(BitcoinTestFramework):
    ...
    def setup_network(self):
        self.wallet = "".join(
            chr(i) for i in range(FILE_CHAR_START, FILE_CHAR_END) if chr(i) not in FILE_CHARS_DISALLOWED
        )
    ...
    def run_test(self):
        if self.is_wallet_compiled():
            ...
            for i, name in enumerate(self.wallet_names):
                self.nodes[i].createwallet(
                    wallet_name=name, descriptors=self.options.descriptors, blank=True, load_on_startup=True
                )
        ...
        blocks = self.generatetoaddress(
            self.nodes[1],
            block_count,
            self.nodes[1].getnewaddress() if self.is_wallet_compiled() else ADDRESS_BCRT1_UNSPENDABLE,
        )
