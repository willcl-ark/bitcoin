...
class RawTransactionsTest(BitcoinTestFramework):
    def set_test_params(self):
        self.num_nodes = 4
        self.extra_args = [["-deprecatedrpc=settxfee"] for i in range(self.num_nodes)]
    ...
    def test_change_position(self):
        ...
        import_res = wwatch.importdescriptors(
            [{"desc": self.nodes[0].getaddressinfo(watchonly_address)["desc"], "timestamp": "now"}]
        )
        assert_equal(import_res[0]["success"], True)
        self.watchonly_utxo = self.create_outpoints(
            self.nodes[0], outputs=[{watchonly_address: self.watchonly_amount}]
        )[0]
        ...
        self.nodes[0].sendtoaddress(
            self.nodes[3].get_wallet_rpc(self.default_wallet_name).getnewaddress(), self.watchonly_amount / 10
        )
    ...
    def test_invalid_option(self):
        ...
        assert_raises_rpc_error(
            ...
            lambda: self.nodes[2].fundrawtransaction(hexstring=rawtx, reserveChangeKey=True),
        )
    ...
    def test_coin_selection(self):
        ...
        assert_raises_rpc_error(
            -4, ERR_NOT_ENOUGH_PRESET_INPUTS, self.nodes[2].fundrawtransaction, rawtx, add_inputs=False
        )
    ...
    def test_spend_2of2(self):
        ...
        funded_psbt = wmulti.walletcreatefundedpsbt(
            inputs=inputs, outputs=outputs, changeAddress=w2.getrawchangeaddress()
        )["psbt"]
    ...
    def test_locked_wallet(self):
        ...
        rawtx = wallet.createrawtransaction(inputs, outputs)
        assert_raises_rpc_error(
            -4, "Transaction needs a change address, but we can't generate it.", wallet.fundrawtransaction, rawtx
        )
    ...
    def test_all_watched_funds(self):
        ...
        result = wwatch.fundrawtransaction(
            rawtx, includeWatching=True, changeAddress=w3.getrawchangeaddress(), subtractFeeFromOutputs=[0]
        )
        res_dec = self.nodes[0].decoderawtransaction(result["hex"])
        assert_equal(len(res_dec["vin"]), 1)
    ...
    def test_option_feerate(self):
        ...
        for param, value in {("fee_rate", 100000), ("feeRate", 1.000)}:
            ...
            assert_raises_rpc_error(
                -3, "Amount out of range", node.fundrawtransaction, rawtx, add_inputs=True, **{param: -1}
            )
            assert_raises_rpc_error(
                -3,
            ...
            for invalid_value in ["", 0.000000001, 1e-09, 1.111111111, 1111111111111111, "31.999999999999999999999"]:
                assert_raises_rpc_error(
                    -3, "Invalid amount", node.fundrawtransaction, rawtx, add_inputs=True, **{param: invalid_value}
                )
        # Test fee_rate values that cannot be represented in sat/vB.
        for invalid_value in [0.0001, 0.00000001, 0.00099999, 31.99999999]:
            assert_raises_rpc_error(
                -3, "Invalid amount", node.fundrawtransaction, rawtx, fee_rate=invalid_value, add_inputs=True
            )
    ...
    def test_option_subtract_fee_from_outputs(self):
        ...
        result = [
            self.nodes[3].fundrawtransaction(rawtx),  # uses self.min_relay_tx_fee (set by settxfee)
        ...
        result = [
            ...
            self.nodes[3].fundrawtransaction(
                rawtx, fee_rate=2 * btc_kvb_to_sat_vb * self.min_relay_tx_fee, subtractFeeFromOutputs=[0]
            ),
        ]
        dec_tx = [self.nodes[3].decoderawtransaction(tx_["hex"]) for tx_ in result]
        output = [d["vout"][1 - r["changepos"]]["value"] for d, r in zip(dec_tx, result)]
        ...
        output = [
            [out["value"] for i, out in enumerate(d["vout"]) if i != r["changepos"]] for d, r in zip(dec_tx, result)
        ]
    ...
    def test_external_inputs(self):
        ...
        assert_raises_rpc_error(
            ...
            wallet.fundrawtransaction,
            raw_tx,
            input_weights=[{"txid": ext_utxo["txid"], "vout": ext_utxo["vout"]}],
        )
        assert_raises_rpc_error(
            -8,
            ...
            wallet.fundrawtransaction,
            raw_tx,
            input_weights=[{"txid": ext_utxo["txid"], "vout": ext_utxo["vout"], "weight": 164}],
        )
        assert_raises_rpc_error(
            -8,
            "Invalid parameter, weight cannot be less than 165",
            wallet.fundrawtransaction,
            raw_tx,
            input_weights=[{"txid": ext_utxo["txid"], "vout": ext_utxo["vout"], "weight": -1}],
        )
        assert_raises_rpc_error(
            -8,
            ...
            wallet.fundrawtransaction,
            raw_tx,
            input_weights=[{"txid": ext_utxo["txid"], "vout": ext_utxo["vout"], "weight": 400001}],
        )

        # But funding should work when the solving data is provided
        funded_tx = wallet.fundrawtransaction(
            raw_tx, solving_data={"pubkeys": [addr_info["pubkey"]], "scripts": [addr_info["embedded"]["scriptPubKey"]]}
        )
        ...
        funded_tx = wallet.fundrawtransaction(
            raw_tx,
            input_weights=[{"txid": ext_utxo["txid"], "vout": ext_utxo["vout"], "weight": input_weight}],
            fee_rate=2,
        )
        signed_tx = wallet.signrawtransactionwithwallet(funded_tx["hex"])
        signed_tx = self.nodes[0].signrawtransactionwithwallet(signed_tx["hex"])
        ...
        funded_tx2 = wallet.fundrawtransaction(
            raw_tx,
            input_weights=[{"txid": ext_utxo["txid"], "vout": ext_utxo["vout"], "weight": low_input_weight}],
            fee_rate=2,
        )
        ...
        funded_tx2 = wallet.fundrawtransaction(
            raw_tx,
            input_weights=[{"txid": ext_utxo["txid"], "vout": ext_utxo["vout"], "weight": high_input_weight}],
            fee_rate=2,
        )
        ...
        funded_tx = wallet.fundrawtransaction(
            raw_tx, input_weights=[{"txid": ext_utxo["txid"], "vout": ext_utxo["vout"], "weight": 255}], fee_rate=2
        )
        funded_tx = wallet.fundrawtransaction(
            raw_tx, input_weights=[{"txid": ext_utxo["txid"], "vout": ext_utxo["vout"], "weight": 65539}], fee_rate=2
        )
    ...
    def test_add_inputs_default_value(self):
        ...
        outputs = {self.nodes[1].getnewaddress(): 8}
        assert_raises_rpc_error(
            -4, ERR_NOT_ENOUGH_PRESET_INPUTS, wallet.walletcreatefundedpsbt, inputs=inputs, outputs=outputs
        )
        ...
        options = {"add_inputs": False}
        assert_raises_rpc_error(
            -4, ERR_NOT_ENOUGH_PRESET_INPUTS, wallet.walletcreatefundedpsbt, inputs=[], outputs=outputs, **options
        )
    ...
    def test_weight_calculation(self):
        ...
        fundedtx = wallet.fundrawtransaction(
            rawtx, fee_rate=10, change_type="bech32", solving_data={"descriptors": [ext_desc]}
        )
    ...
    def test_weight_limits(self):
        ...
        assert_raises_rpc_error(
            -4, "Transaction too large", wallet.fundrawtransaction, hexstring=rawtx, input_weights=input_weights
        )
    ...
    def test_feerate_rounding(self):
        ...
        rawtx = w.createrawtransaction(
            inputs=[], outputs=[{self.nodes[0].getnewaddress(address_type="bech32"): 1 - 0.00000202}]
        )
