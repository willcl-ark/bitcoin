...
class NULLDUMMYTest(BitcoinTestFramework):
    ...
    def run_test(self):
        ...
        test1txs.append(
            self.create_transaction(
                txid=txid1, input_details=ms_unlock_details, addr=self.ms_address, amount=48, privkey=self.privkey
            )
        )
        ...
        test2tx = self.create_transaction(
            txid=txid2, input_details=ms_unlock_details, addr=self.ms_address, amount=47, privkey=self.privkey
        )
        invalidate_nulldummy_tx(test2tx)
        assert_raises_rpc_error(
            -26, NULLDUMMY_ERROR, self.nodes[0].sendrawtransaction, test2tx.serialize_with_witness().hex(), 0
        )
        ...
        assert_raises_rpc_error(
            -26, NULLDUMMY_ERROR, self.nodes[0].sendrawtransaction, test4tx.serialize_with_witness().hex(), 0
        )
        ...
        assert_raises_rpc_error(
            -26, NULLDUMMY_ERROR, self.nodes[0].sendrawtransaction, test5tx.serialize_with_witness().hex(), 0
        )
