...
class BlockchainTest(BitcoinTestFramework):
    ...
    def _test_prune_disk_space(self):
        self.log.info("Test that a manually pruned node does not run into integer overflow on first start up")
    ...
    def _test_getdeploymentinfo(self):
        ...
        self.check_signalling_deploymentinfo_result(
            self.nodes[0].getdeploymentinfo(), gbci207["blocks"], gbci207["bestblockhash"], "started"
        )
        ...
        self.check_signalling_deploymentinfo_result(
            self.nodes[0].getdeploymentinfo(), gbci287["blocks"], gbci287["bestblockhash"], "locked_in"
        )
    ...
    def _test_verificationprogress(self):
        ...
        self.nodes[0].submitheader(
            self.generateblock(self.nodes[0], output="raw(55)", transactions=[], submit=False, sync_fun=self.no_op)[
                "hex"
            ]
        )
        assert_greater_than(1, self.nodes[0].getblockchaininfo()["verificationprogress"])
    ...
    def _test_getchaintxstats(self):
        ...
        assert_raises_rpc_error(
            -3, "JSON value of type string is not of expected type number", self.nodes[0].getchaintxstats, ""
        )
        assert_raises_rpc_error(
        ...
        assert_raises_rpc_error(
            -3, "JSON value of type number is not of expected type string", self.nodes[0].getchaintxstats, blockhash=0
        )
        assert_raises_rpc_error(
            -8, "blockhash must be of length 64 (not 1, for '0')", self.nodes[0].getchaintxstats, blockhash="0"
        )
        assert_raises_rpc_error(
            -8,
    ...
    def _test_getblockheader(self):
        ...
        assert_raises_rpc_error(
            -8, "hash must be of length 64 (not 8, for 'nonsense')", node.getblockheader, "nonsense"
        )
        assert_raises_rpc_error(
            -8,
    ...
    def _test_waitforblockheight(self):
        ...
        def assert_waitforheight(height, timeout=2):
            assert_equal(node.waitforblockheight(height=height, timeout=timeout)["height"], current_height)
    ...
    def _test_getblock(self):
        ...
        assert_raises_rpc_error(
            -3, "JSON value of type string is not of expected type number", node.getblock, blockhash, "2"
        )
        ...
        assert_raises_rpc_error(
            ...
            lambda: node.getblock(blockhash, 2),
        )
        assert_raises_rpc_error(
            -32603,
            "Undo data expected but can't be read. This could be due to disk corruption or a conflict with a pruning event.",
            lambda: node.getblock(blockhash, 3),
        )
        ...
        block_noundo = create_block(
            block.sha256, create_coinbase(current_height + 2, nValue=100), block_time + 1, txlist=[tx]
        )
        block_noundo.solve()
