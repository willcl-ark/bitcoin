...
class PSBTTest(BitcoinTestFramework):
    ...
    def test_psbt_incomplete_after_invalid_modification(self):
        ...
        psbt = wallet.createpsbt(
            [{"txid": utxos[0]["txid"], "vout": utxos[0]["vout"]}], [{wallet.getnewaddress(): 0.9999}]
        )
        ...
        raw = wallet.createrawtransaction(
            [{"txid": utxos[0]["txid"], "vout": utxos[0]["vout"]}], [{substitute_addr: 0.9999}]
        )
    ...
    def test_utxo_conversion(self):
        ...
        import_res = wonline.importdescriptors(
            [{"desc": offline_node.getaddressinfo(offline_addr)["desc"], "timestamp": "now"}]
        )
        ...
        raw = wonline.createrawtransaction(
            [{"txid": utxos[0]["txid"], "vout": utxos[0]["vout"]}], [{online_addr: 0.9999}]
        )
    ...
    def test_input_confs_control(self):
        ...
        assert_raises_rpc_error(
            -26, "bad-txns-spends-conflicting-tx", self.nodes[0].sendrawtransaction, signed_invalid["hex"]
        )
    ...
    def test_sighash_mismatch(self):
        ...
        for sighash in ["DEFAULT", "ALL", "NONE", "SINGLE", "NONE|ANYONECANPAY", "SINGLE|ANYONECANPAY", None]:
            assert_raises_rpc_error(
                -22,
                "Specified sighash value does not match value stored in PSBT",
                self.nodes[0].descriptorprocesspsbt,
                psbt,
                descs,
                sighash,
            )
    ...
    def test_sighash_adding(self):
        ...
        desc_psbt = self.nodes[0].descriptorprocesspsbt(
            psbt=psbt, descriptors=descs, sighashtype="ALL|ANYONECANPAY", finalize=False
        )["psbt"]
    ...
    def run_test(self):
        ...
        assert_raises_rpc_error(
            -4,
            f"Maximum transaction weight must be between {min_tx_weight} and {MAX_STANDARD_TX_WEIGHT}",
        ...
        assert_raises_rpc_error(
            ...
            dest_arg,
            0,
            {"max_tx_weight": MAX_STANDARD_TX_WEIGHT + 1},
        )
        ...
        assert_raises_rpc_error(
            ...
            dest_arg_large,
            0,
            {"max_tx_weight": large_tx_weight_without_inputs},
        )
        ...
        assert_raises_rpc_error(
            ...
            dest_arg_large,
            0,
            {"max_tx_weight": (large_tx_vsize_without_inputs + 1) * WITNESS_SCALE_FACTOR},
        )
        ...
        assert_raises_rpc_error(
            ...
            dest_arg_large,
            0,
            {"max_tx_weight": (large_tx_vsize_with_change) * WITNESS_SCALE_FACTOR},
        )

        max_tx_weight_sufficient = 1000  # 1k vbytes is enough
        psbt = self.nodes[0].walletcreatefundedpsbt(
            outputs=dest_arg, locktime=0, options={"max_tx_weight": max_tx_weight_sufficient}
        )["psbt"]
        ...
        self.nodes[0].walletcreatefundedpsbt(
            [{"txid": utxo1["txid"], "vout": utxo1["vout"]}], {self.nodes[2].getnewaddress(): 1}, 0
        )
        ...
        rawtx = self.nodes[0].createrawtransaction(
            [], {p2sh: 10, p2wsh: 10, p2wpkh: 10, p2sh_p2wsh: 10, p2sh_p2wpkh: 10, p2pkh: 10}
        )
        ...
        for param, zero_value in product(["fee_rate", "feeRate"], [0, 0.000, 0.00000000, "0", "0.000", "0.00000000"]):
            assert_equal(
                0,
                self.nodes[1].walletcreatefundedpsbt(inputs, outputs, 0, {param: zero_value, "add_inputs": True})[
                    "fee"
                ],
            )
        ...
        for mode in ["unset", "economical", "conservative"]:
            ...
            for k, v in {"string": "", "object": {"foo": "bar"}}.items():
                assert_raises_rpc_error(
                    ...
                    outputs,
                    0,
                    {"estimate_mode": mode, "conf_target": v, "add_inputs": True},
                )
            for n in [-1, 0, 1009]:
                assert_raises_rpc_error(
                    -8,
                    ...
                    outputs,
                    0,
                    {"estimate_mode": mode, "conf_target": n, "add_inputs": True},
                )
        ...
        psbtx = wmulti.walletcreatefundedpsbt(
            ...
            outputs={self.nodes[1].getnewaddress(): 29.99},
            changeAddress=self.nodes[1].getrawchangeaddress(),
        )["psbt"]
        ...
        rawtx = self.nodes[1].createrawtransaction(
            [{"txid": txid, "vout": p2wpkh_pos}], {self.nodes[1].getnewaddress(): 9.99}
        )
        ...
        )  # permitsigdata=False by default
        assert_raises_rpc_error(
            -22,
            "Inputs must not have scriptSigs and scriptWitnesses",
            self.nodes[0].converttopsbt,
        ...
        self.sync_mempools()
        assert_raises_rpc_error(
            -4, "Insufficient funds", wunsafe.walletcreatefundedpsbt, [], [{self.nodes[0].getnewaddress(): 1}]
        )
        ...
        # Open the data file
        with open(
            os.path.join(os.path.dirname(os.path.realpath(__file__)), "data/rpc_psbt.json"), encoding="utf-8"
        ) as f:
            d = json.load(f)
            invalids = d["invalid"]
        ...
        for creator in creators:
            created_tx = self.nodes[0].createpsbt(
                inputs=creator["inputs"], outputs=creator["outputs"], replaceable=False
            )
        ...
        assert_raises_rpc_error(
            -22, "TX decode failed invalid base64", self.nodes[0].decodepsbt, ";definitely not base64;"
        )
        ...
        test_psbt_input_keys(
            decoded["inputs"][2], ["non_witness_utxo", "witness_utxo", "bip32_derivs", "redeem_script"]
        )
        ...
        assert (
            ...
            and analyzed["inputs"][0]["missing"]["signatures"][0] == addrinfo["embedded"]["witness_program"]
        )
        ...
        assert (
            analyzed["inputs"][0]["has_utxo"] and analyzed["inputs"][0]["is_final"] and analyzed["next"] == "extractor"
        )
        ...
        assert_raises_rpc_error(
            -4,
            "Not solvable pre-selected input COutPoint(%s, %s)" % (ext_utxo["txid"][0:10], ext_utxo["vout"]),
            wallet.walletcreatefundedpsbt,
            [ext_utxo],
        ...
        assert_raises_rpc_error(
            -8, "'all' is not a valid sighash parameter.", self.nodes[0].walletprocesspsbt, psbt, sighashtype="all"
        )
        ...
        tx = CTransaction()
        tx.vin = [
            CTxIn(outpoint=COutPoint(hash=int("aa" * 32, 16), n=0), scriptSig=b""),
        ...
        psbt.g = PSBTMap({PSBT_GLOBAL_UNSIGNED_TX: tx.serialize()})
        psbt.i = [
            PSBTMap({bytes([PSBT_IN_RIPEMD160]) + hash_ripemd160: preimage_ripemd160}),
        ...
        alt_psbt = self.nodes[2].descriptorprocesspsbt(psbt=psbt, descriptors=[alt_descriptor], sighashtype="ALL")[
            "psbt"
        ]
        ...
        processed_psbt = self.nodes[2].descriptorprocesspsbt(
            psbt=psbt, descriptors=[descriptor], sighashtype="ALL", bip32derivs=True, finalize=False
        )
        decoded = self.nodes[2].decodepsbt(processed_psbt["psbt"])
        test_psbt_input_keys(
            decoded["inputs"][0], ["witness_utxo", "non_witness_utxo", "partial_signatures", "bip32_derivs"]
        )
        ...
        processed_psbt = self.nodes[2].descriptorprocesspsbt(
            psbt=psbt, descriptors=[descriptor], sighashtype="ALL", bip32derivs=False, finalize=True
        )
        decoded = self.nodes[2].decodepsbt(processed_psbt["psbt"])
