...
class MempoolPackageLimitsTest(BitcoinTestFramework):
    ...
    def test_anc_size_limits(self):
        ...
        pc_tx = self.wallet.create_self_transfer_multi(
            utxos_to_spend=parent_utxos, fee_per_output=high_fee, target_weight=target_weight
        )
    ...
    def test_desc_size_limits(self):
        ...
        ma_tx = self.wallet.create_self_transfer_multi(
            num_outputs=2, fee_per_output=high_fee // 2, target_weight=target_weight
        )
        ...
        for j in range(2):  # Two legs (left and right)
            ...
            package_tx = self.wallet.create_self_transfer(
                utxo_to_spend=mempool_tx["new_utxo"], target_weight=target_weight
            )
