...
class P2PCompactBlocksBlocksOnly(BitcoinTestFramework):
    ...
    def run_test(self):
        ...
        self.log.info(
            "Test that -blocksonly nodes send getdata(BLOCK) instead of getdata(CMPCT) in BIP152 low bandwidth mode"
        )
        ...
        assert_equal(
            p2p_conn_blocksonly.last_message["getdata"].inv, [CInv(MSG_BLOCK | MSG_WITNESS_FLAG, block1.sha256)]
        )
