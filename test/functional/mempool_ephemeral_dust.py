...
class EphemeralDustTest(BitcoinTestFramework):
    ...
    def add_output_to_create_multi_result(self, result, output_value=0):
        ...
        result["new_utxos"].append(
            {
                "txid": new_txid,
                "vout": len(result["tx"].vout) - 1,
                "value": Decimal(output_value) / COIN,
                "height": 0,
                "coinbase": False,
                "confirmations": 0,
            }
    ...
    def create_ephemeral_dust_package(
        self, *, tx_version, dust_tx_fee=0, dust_value=0, num_dust_outputs=1, extra_sponsors=None
    ):
    ...
    def test_non_truc(self):
        self.log.info(
            "Test that v2 dust-having transaction is rejected even if spent, because of min relay requirement"
        )
    ...
    def test_unspent_ephemeral(self):
        ...
        unspent_sweep_tx = self.wallet.create_self_transfer_multi(
            fee_per_output=2000, utxos_to_spend=[dusty_tx["new_utxos"][0]], version=3
        )
        assert_greater_than(unspent_sweep_tx["fee"], sweep_tx["fee"])
        res = self.nodes[0].submitpackage([dusty_tx["hex"], unspent_sweep_tx["hex"]])
        assert_equal(
            res["tx-results"][unspent_sweep_tx["wtxid"]]["error"],
            f"missing-ephemeral-spends, tx {unspent_sweep_tx['txid']} (wtxid={unspent_sweep_tx['wtxid']}) did not spend parent's ephemeral dust",
        )
        assert_raises_rpc_error(
            -26,
            f"missing-ephemeral-spends, tx {unspent_sweep_tx['txid']} (wtxid={unspent_sweep_tx['wtxid']}) did not spend parent's ephemeral dust",
            self.nodes[0].sendrawtransaction,
            unspent_sweep_tx["hex"],
        )
        assert_mempool_contents(self, self.nodes[0], expected=[dusty_tx["tx"], sweep_tx["tx"]])

        # Spend works with dust spent
        sweep_tx_2 = self.wallet.create_self_transfer_multi(
            fee_per_output=2000, utxos_to_spend=dusty_tx["new_utxos"], version=3
        )
        ...
        sweep_tx = self.wallet.create_self_transfer_multi(
            utxos_to_spend=[dusty_tx["new_utxos"][1], second_coin], version=3
        )
    ...
    def test_reorgs(self):
        ...
        sweep_tx = self.wallet.create_self_transfer_multi(
            fee_per_output=0, utxos_to_spend=[dusty_tx["new_utxos"][0]], version=3
        )
        ...
        block_res = self.generateblock(
            self.nodes[0], self.wallet.get_address(), [dusty_tx["hex"], sweep_tx["hex"]], sync_fun=self.no_op
        )
        self.nodes[0].invalidateblock(block_res["hash"])
        assert_mempool_contents(self, self.nodes[0], expected=[dusty_tx["tx"]], sync=False)
        ...
        sweep_tx_2 = self.wallet.create_self_transfer_multi(
            fee_per_output=0, utxos_to_spend=dusty_tx["new_utxos"], version=3
        )
        ...
        reconsider_block_res = self.generateblock(
            self.nodes[0], self.wallet.get_address(), [dusty_tx["hex"], sweep_tx_2["hex"]], sync_fun=self.no_op
        )
        ...
        block_res = self.generateblock(
            self.nodes[0], self.wallet.get_address(), [multi_dusty_tx["hex"]], sync_fun=self.no_op
        )
        self.nodes[0].invalidateblock(block_res["hash"])
        assert_equal(self.nodes[0].getrawmempool(), [])
        ...
        block_res = self.generateblock(
            self.nodes[0], self.wallet.get_address(), [dusty_fee_tx["hex"]], sync_fun=self.no_op
        )
        self.nodes[0].invalidateblock(block_res["hash"])
        assert_equal(self.nodes[0].getrawmempool(), [])
    ...
    def test_no_minrelay_fee(self):
        ...
        insufficient_sweep_tx = self.wallet.create_self_transfer_multi(
            fee_per_output=25000, utxos_to_spend=all_parent_utxos[:-1], version=2
        )

        res = self.nodes[0].submitpackage([dusty_tx["hex"] for dusty_tx in dusty_txs] + [insufficient_sweep_tx["hex"]])
        assert_equal(res["package_msg"], "transaction failed")
        assert_equal(
            res["tx-results"][insufficient_sweep_tx["wtxid"]]["error"],
            f"missing-ephemeral-spends, tx {insufficient_sweep_tx['txid']} (wtxid={insufficient_sweep_tx['wtxid']}) did not spend parent's ephemeral dust",
        )
        ...
        sweep_all_but_one_tx = self.wallet.create_self_transfer_multi(
            fee_per_output=20000, utxos_to_spend=all_parent_utxos[:-2] + [B_coin], version=2
        )
        res = self.nodes[0].submitpackage(
            [dusty_tx["hex"] for dusty_tx in dusty_txs[:-1]] + [sweep_all_but_one_tx["hex"]]
        )
        assert_equal(res["package_msg"], "success")
        assert_mempool_contents(
            self, self.nodes[0], expected=[dusty_tx["tx"] for dusty_tx in dusty_txs] + [sweep_all_but_one_tx["tx"]]
        )
        ...
        assert_mempool_contents(
            self, self.nodes[0], expected=[dusty_tx["tx"] for dusty_tx in dusty_txs] + [cancel_sweep["tx"]]
        )

        # Sweeps all dust, where all dusty txs are already in-mempool
        sweep_tx = self.wallet.create_self_transfer_multi(
            fee_per_output=25000, utxos_to_spend=all_parent_utxos, version=2
        )
        ...
        assert_mempool_contents(
            self,
            self.nodes[0],
            expected=[dusty_tx["tx"] for dusty_tx in dusty_txs] + [sweep_tx["tx"], cancel_sweep["tx"]],
        )
