...
class UpgradeWalletTest(BitcoinTestFramework):
    ...
    def run_test(self):
        ...
        self.generatetoaddress(
            node_master, COINBASE_MATURITY + 1, v16_3_address, sync_fun=lambda: self.dumb_sync_blocks()
        )
        ...
        def copy_split_hd():
            ...
            shutil.rmtree(node_master_wallet_dir)
            os.mkdir(node_master_wallet_dir)
            shutil.copy(split_hd_wallet, os.path.join(node_master_wallet_dir, "wallet.dat"))
            node_master.loadwallet(self.default_wallet_name)
        ...
        for k, old_v in old_kvs.items():
            if k.startswith(b"\x07keymeta"):
                new_ver, new_create_time, new_kp_str, new_seed_id, new_fpr, new_path_len, new_path, new_has_key_orig = (
                    deser_keymeta(BytesIO(new_kvs[k]))
                )
                old_ver, old_create_time, old_kp_str, old_seed_id, old_fpr, old_path_len, old_path, old_has_key_orig = (
        ...
        if self.is_bdb_compiled():
            ...
            self.nodes[0].createwallet(
                wallet_name="privkeys_disabled_upgrade", disable_private_keys=True, descriptors=False
            )
