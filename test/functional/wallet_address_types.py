    ...
    def test_desc(self, node, address, multisig, typ, utxo):
        ...
        for deriv in decode["inputs"][0]["bip32_derivs"]:
            ...
            key_descs[deriv["pubkey"]] = (
                "[" + deriv["master_fingerprint"] + deriv["path"][1:].replace("'", "h") + "]" + deriv["pubkey"]
            )
        ...
        if not multisig and typ == "legacy":
            ...
            assert_equal(
                info["desc"],
                descsum_create("sh(multi(2,%s,%s))" % (key_descs[info["pubkeys"][0]], key_descs[info["pubkeys"][1]])),
            )
        elif typ == "p2sh-segwit":
            # P2SH-P2WSH-multisig
            assert_equal(
                ...
                descsum_create(
                    "sh(wsh(multi(2,%s,%s)))"
                    % (key_descs[info["embedded"]["pubkeys"][0]], key_descs[info["embedded"]["pubkeys"][1]])
                ),
            )
        elif typ == "bech32":
            # P2WSH-multisig
            assert_equal(
                info["desc"],
                descsum_create("wsh(multi(2,%s,%s))" % (key_descs[info["pubkeys"][0]], key_descs[info["pubkeys"][1]])),
            )
        else:
            # Unknown type
            assert False
    ...
    def run_test(self):
        ...
        if not self.options.descriptors:
            ...
            for node in range(4):
                self.test_address(
                    node,
                    self.nodes[node].addmultisigaddress(2, [uncompressed_1, uncompressed_2])["address"],
                    True,
                    "legacy",
                )
                self.test_address(
                    node,
                    self.nodes[node].addmultisigaddress(2, [compressed_1, uncompressed_2])["address"],
                    True,
                    "legacy",
                )
                self.test_address(
                    node,
                    self.nodes[node].addmultisigaddress(2, [uncompressed_1, compressed_2])["address"],
                    True,
                    "legacy",
                )
            # addmultisigaddress with all compressed keys should return the appropriate address type (even when the keys are not ours).
            self.test_address(
                0, self.nodes[0].addmultisigaddress(2, [compressed_1, compressed_2])["address"], True, "legacy"
            )
            self.test_address(
                1, self.nodes[1].addmultisigaddress(2, [compressed_1, compressed_2])["address"], True, "p2sh-segwit"
            )
            self.test_address(
                2, self.nodes[2].addmultisigaddress(2, [compressed_1, compressed_2])["address"], True, "p2sh-segwit"
            )
            self.test_address(
                3, self.nodes[3].addmultisigaddress(2, [compressed_1, compressed_2])["address"], True, "bech32"
            )
        ...
        assert_raises_rpc_error(
            -5, "Unknown address type ''", self.nodes[3].addmultisigaddress, 2, [compressed_1, compressed_2], None, ""
        )
        ...
        if self.options.descriptors:
            ...
            assert_raises_rpc_error(
                -8, "Legacy wallets cannot provide bech32m addresses", self.nodes[0].getnewaddress, "", "bech32m"
            )
            assert_raises_rpc_error(
                -8, "Legacy wallets cannot provide bech32m addresses", self.nodes[0].getrawchangeaddress, "bech32m"
            )
