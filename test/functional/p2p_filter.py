...
class FilterTest(BitcoinTestFramework):
    ...
    def test_msg_mempool(self):
        ...
        rel_txid = self.wallet.send_to(
            from_node=self.nodes[0], scriptPubKey=filter_peer.watch_script_pubkey, amount=1 * COIN
        )["txid"]
    ...
    def test_filter(self, filter_peer):
        ...
        filter_peer.send_and_ping(msg_filterclear())
        for _ in range(5):
            txid = self.wallet.send_to(from_node=self.nodes[0], scriptPubKey=getnewdestination()[1], amount=7 * COIN)[
                "txid"
            ]
            filter_peer.wait_for_tx(txid)
