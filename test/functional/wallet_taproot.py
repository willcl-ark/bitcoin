...
class WalletTaprootTest(BitcoinTestFramework):
    ...
    def do_test_addr(self, comment, pattern, privmap, treefn, keys):
        ...
        self.nodes[0].createwallet(
            wallet_name=f"pubs_tr_enabled_{wallet_uuid}", descriptors=True, blank=True, disable_private_keys=True
        )
        self.nodes[0].createwallet(
            wallet_name=f"addr_gen_{wallet_uuid}", descriptors=True, disable_private_keys=True, blank=True
        )
    ...
    def do_test_sendtoaddress(self, comment, pattern, privmap, treefn, keys_pay, keys_change):
        ...
        result = rpc_online.importdescriptors(
            [{"desc": desc_change, "active": True, "timestamp": "now", "internal": True}]
        )
        assert result[0]["success"]
        address_type = "bech32m" if "tr" in pattern else "bech32"
        for i in range(4):
    ...
    def do_test_psbt(self, comment, pattern, privmap, treefn, keys_pay, keys_change):
        ...
        wallet_uuid = uuid.uuid4().hex
        self.nodes[0].createwallet(
            wallet_name=f"psbt_online_{wallet_uuid}", descriptors=True, disable_private_keys=True, blank=True
        )
        ...
        result = psbt_online.importdescriptors(
            [{"desc": desc_change_pub, "active": True, "timestamp": "now", "internal": True}]
        )
        assert result[0]["success"]
        ...
        for key in keys_pay + keys_change:
            result = key_only_wallet.importdescriptors(
                [{"desc": descsum_create(f"wpkh({key['xprv']}/*)"), "timestamp": "now"}]
            )
            assert result[0]["success"]
        address_type = "bech32m" if "tr" in pattern else "bech32"
        for i in range(4):
            ...
            psbt = psbt_online.walletcreatefundedpsbt(
                ...
                None,
                {"subtractFeeFromOutputs": [0], "fee_rate": 200, "change_type": address_type},
            )["psbt"]
    ...
    def run_test(self):
        ...
        self.do_test("tr(H,XPRV)", "tr($H,pk($1/*))", [True], lambda k1: (key(H_POINT), [pk(k1)]))
        ...
        self.do_test("wsh(multi(1,XPRV,XPUB))", "wsh(multi(1,$1/*,$2/*))", [True, False], None)
        self.do_test(
            "tr(XPRV,{XPUB,XPUB})",
            "tr($1/*,{pk($2/*),pk($2/*)})",
            [True, False],
        ...
        self.do_test(
            ...
            lambda k1, k2, k3: (
                key(k1),
                [[pk(H_POINT), [pk(H_POINT), pk(k2)]], [pk(H_POINT), [pk(H_POINT), [pk(H_POINT), pk(k3)]]]],
            ),
        )
        self.do_test(
            ...
            lambda k1, k2: (
                key(k1),
                [pk(k2), [[pk(k2), [pk(H_POINT), pk(H_POINT)]], [[pk(H_POINT), pk(H_POINT)], pk(k2)]]],
            ),
        )
        self.do_test(
            "tr(H,multi_a(1,XPRV))", "tr($H,multi_a(1,$1/*))", [True], lambda k1: (key(H_POINT), [multi_a(1, [k1])])
        )
        self.do_test(
        ...
        self.do_test(
            ...
            lambda k1, k2: (
                key(k2),
                [multi_a(1, ([H_POINT] * rnd_pos) + [k1] + ([H_POINT] * (MAX_PUBKEYS_PER_MULTI_A - 1 - rnd_pos)))],
            ),
        )
