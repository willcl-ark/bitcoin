...
class GetblockstatsTest(BitcoinTestFramework):
    ...
    def add_options(self, parser):
        parser.add_argument(
            "--gen-test-data", dest="gen_test_data", default=False, action="store_true", help="Generate test data"
        )
        parser.add_argument(
            "--test-data",
            dest="test_data",
    ...
    def run_test(self):
        ...
        for stat in expected_keys:
            for i in range(self.max_stat_pos + 1):
                ...
                if result[stat] != self.expected_stats[i][stat]:
                    self.log.info(
                        "result[%s] (%d) failed, %r != %r" % (stat, i, result[stat], self.expected_stats[i][stat])
                    )
        ...
        assert_raises_rpc_error(
            -8, "Target block height %d is negative" % (-1), self.nodes[0].getblockstats, hash_or_height=-1
        )
        ...
        assert_raises_rpc_error(
            -1, "Block not available (not fully downloaded)", lambda: self.nodes[0].getblockstats(block["hash"])
        )
