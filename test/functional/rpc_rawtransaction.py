...
class RawTransactionsTest(BitcoinTestFramework):
    ...
    def getrawtransaction_tests(self):
        ...
        for n in [0, 2]:
            ...
            for value in ["True", "False"]:
                assert_raises_rpc_error(
                    -3, "not of expected type number", self.nodes[n].getrawtransaction, txid=txId, verbose=value
                )
                assert_raises_rpc_error(
                    -3, "not of expected type number", self.nodes[n].getrawtransaction, txid=txId, verbosity=value
                )
        ...
        for n in [0, 2]:
            ...
            assert_raises_rpc_error(
                -5, "No such transaction found", self.nodes[n].getrawtransaction, txid=tx, blockhash=block2
            )
    ...
    def createrawtransaction_tests(self):
        ...
        assert_raises_rpc_error(
            -3, "JSON value of type string is not of expected type array", self.nodes[0].createrawtransaction, "foo", {}
        )
        assert_raises_rpc_error(
            -3,
        ...
        assert_raises_rpc_error(
            -3, "JSON value of type null is not of expected type string", self.nodes[0].createrawtransaction, [{}], {}
        )
        assert_raises_rpc_error(
            -8,
        ...
        assert_raises_rpc_error(
            -8, "Invalid parameter, missing vout key", self.nodes[0].createrawtransaction, [{"txid": TXID}], {}
        )
        assert_raises_rpc_error(
            -8,
        ...
        assert_raises_rpc_error(
            -8, "Data must be hexadecimal string", self.nodes[0].createrawtransaction, [], {"data": "foo"}
        )
        ...
        assert_raises_rpc_error(
            ...
            self.nodes[0].createrawtransaction,
            [{"txid": TXID, "vout": 0, "sequence": MAX_BIP125_RBF_SEQUENCE + 1}],
            {},
            0,
            True,
        )
        ...
        assert_raises_rpc_error(
            -8, "Invalid parameter, locktime out of range", self.nodes[0].createrawtransaction, [], {}, -1
        )
        assert_raises_rpc_error(
            -8, "Invalid parameter, locktime out of range", self.nodes[0].createrawtransaction, [], {}, 4294967296
        )
        ...
        tx = tx_from_hex(
            self.nodes[2].createrawtransaction(
                inputs=[{"txid": TXID, "vout": 9}], outputs=OrderedDict([(address, 99), (address2, 99)])
            )
        )
        ...
        assert_equal(
            tx.serialize().hex(),
            self.nodes[2].createrawtransaction(
                inputs=[{"txid": TXID, "vout": 9}], outputs=[{address: 99}, {address2: 99}]
            ),
        )
        # Multiple mixed outputs
        tx = tx_from_hex(
            self.nodes[2].createrawtransaction(
                inputs=[{"txid": TXID, "vout": 9}], outputs=multidict([(address, 99), (address2, 99), ("data", "99")])
            )
        )
        ...
        assert_equal(
            tx.serialize().hex(),
            self.nodes[2].createrawtransaction(
                inputs=[{"txid": TXID, "vout": 9}], outputs=[{address: 99}, {address2: 99}, {"data": "99"}]
            ),
        )
    ...
    def decoderawtransaction_tests(self):
        ...
        assert_raises_rpc_error(
            -22, "TX decode failed", self.nodes[0].decoderawtransaction, encrawtx, False
    ...
    def raw_multisig_transaction_legacy_tests(self):
        ...
        assert_raises_rpc_error(
            -5,
            'Pubkey "01020304" must have a length of either 33 or 65 bytes',
        ...
        assert_raises_rpc_error(
            -5, f'Pubkey "{addr1}" must be a hex string', self.nodes[0].createmultisig, 2, [addr1Obj["pubkey"], addr1]
        )
        ...
        self.generate(self.nodes[0], 1)
        assert_equal(
            self.nodes[0].getbalance(), bal + Decimal("50.00000000") + Decimal("2.19000000")
        )  # block reward + tx
        ...
        assert_raises_rpc_error(
            ...
            [rawTxPartialSigned1["hex"], rawTxPartialSigned2["hex"] + "00"],
        )
