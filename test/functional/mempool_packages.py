...
class MempoolPackagesTest(BitcoinTestFramework):
    ...
    def run_test(self):
        ...
        self.wait_until(
            lambda: len(self.nodes[1].getrawmempool()) == CUSTOM_ANCESTOR_LIMIT + 1 + CUSTOM_DESCENDANT_LIMIT,
            timeout=10,
        )
        mempool0 = self.nodes[0].getrawmempool(False)
        mempool1 = self.nodes[1].getrawmempool(False)
        ...
        tx7 = self.wallet.send_self_transfer_chain(
            from_node=self.nodes[0], utxo_to_spend=tx0["new_utxos"][1], chain_length=6
        )[-1]
        ...
        self.wallet.send_self_transfer_multi(
            from_node=self.nodes[0], utxos_to_spend=[tx1["new_utxo"], tx7["new_utxo"]], fee_per_output=40000
        )
        self.sync_mempools()
