...
class RPCBindTest(BitcoinTestFramework):
    ...
    def run_allowip_test(self, allow_ips, rpchost, rpcport):
        ...
        node_args = (
            ...
            + ["-rpcbind=" + addr for addr in ["127.0.0.1", "%s:%d" % (rpchost, rpcport)]]
        )  # Bind to localhost as well so start_nodes doesn't hang
        self.nodes[0].rpchost = None
        self.start_nodes([node_args])
        # connect to node through non-loopback interface
        node = get_rpc_proxy(
            rpc_url(self.nodes[0].datadir_path, 0, self.chain, "%s:%d" % (rpchost, rpcport)),
            0,
        ...
        if not self.options.run_nonloopback:
            ...
            if self.options.run_ipv4:
                self.run_invalid_bind_test(
                    ["127.0.0.1"], ["127.0.0.1:notaport", "127.0.0.1:-18443", "127.0.0.1:0", "127.0.0.1:65536"]
                )
    ...
    def _run_loopback_tests(self):
        if self.options.run_ipv4:
            ...
            self.run_bind_test(["127.0.0.1"], "127.0.0.1", ["127.0.0.1"], [("127.0.0.1", self.defaultport)])
            # check only IPv4 localhost (explicit) with alternative port
            self.run_bind_test(["127.0.0.1"], "127.0.0.1:32171", ["127.0.0.1:32171"], [("127.0.0.1", 32171)])
            ...
            self.run_bind_test(None, "127.0.0.1", [], [("127.0.0.1", self.defaultport), ("::1", self.defaultport)])
            # check default with rpcallowip (IPv4 and IPv6 localhost)
            self.run_bind_test(
                ["127.0.0.1"], "127.0.0.1", [], [("127.0.0.1", self.defaultport), ("::1", self.defaultport)]
            )
            # check only IPv6 localhost (explicit)
            self.run_bind_test(["[::1]"], "[::1]", ["[::1]"], [("::1", self.defaultport)])
