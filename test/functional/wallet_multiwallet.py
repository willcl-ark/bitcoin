...
class MultiWalletTest(BitcoinTestFramework):
    ...
    def run_test(self):
        ...
        assert_raises_rpc_error(
            err_code,
            "filesystem error:" if platform.system() != "Windows" else "create_directories:",
        ...
        self.nodes[0].assert_start_raises_init_error(
            ["-upgradewallet"], "Error: Error parsing command line arguments: Invalid parameter -upgradewallet"
        )
        ...
        for _ in range(3):
            n = (
                ...
                else get_rpc_proxy(node.url, 1, timeout=600, coveragedir=node.coverage_dir)
            )
        ...
        assert_raises_rpc_error(
            -18,
            "Wallet file verification failed. Failed to load database path '{}'. Path does not exist.".format(path),
        ...
        assert_raises_rpc_error(
            -4,
            "Failed to create database path '{}'. Database already exists.".format(path),
            self.nodes[0].createwallet,
            "w2",
        )
        ...
        assert_raises_rpc_error(
            -3, "JSON value of type null is not of expected type string", self.nodes[0].unloadwallet
        )
        assert_raises_rpc_error(
            -18, "Requested wallet does not exist or is not loaded", self.nodes[0].unloadwallet, "dummy"
        )
        assert_raises_rpc_error(
            -18, "Requested wallet does not exist or is not loaded", node.get_wallet_rpc("dummy").unloadwallet
        )
        (
            assert_raises_rpc_error(
                -8, "RPC endpoint wallet and wallet_name parameter specify different wallets", w1.unloadwallet, "w2"
            ),
        )
        ...
        if self.options.descriptors:
            ...
            assert_raises_rpc_error(
                -4, "Error initializing wallet database environment", self.nodes[1].loadwallet, wallet
            )
