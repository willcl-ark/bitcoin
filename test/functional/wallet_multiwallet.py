...
class MultiWalletTest(BitcoinTestFramework):
    ...
    def add_options(self, parser):
        ...
        parser.add_argument(
            '--data_wallets_dir',
            default=Path(__file__).resolve().parent / 'data/wallets/',
    ...
    def run_test(self):
        ...
        data_dir = lambda *p: node.chain_path / Path(*p)
        ...
        self.stop_nodes()
        assert_equal(wallet_dir(self.default_wallet_name, self.wallet_data_filename).is_file(), True)

        # create symlink to verify wallet directory path can be referenced through symlink
        wallet_dir("w7").mkdir()
        wallet_dir("w7_symlink").symlink_to(wallet_dir("w7"))
        wallet_dir("recursive_dir_symlink").symlink_to(wallet_dir(".."))
        wallet_dir("self_walletdat_symlink").symlink_to(wallet_dir("wallet.dat"))
        ...
        self.stop_nodes()
        empty_wallet = self.options.tmpdir / "empty.dat"
        wallet_file("empty").rename(empty_wallet)
        shutil.rmtree(wallet_dir("empty"))
        empty_created_wallet = self.options.tmpdir / "empty.created.dat"
        wallet_dir("created", self.wallet_data_filename).rename(empty_created_wallet)
        shutil.rmtree(wallet_dir("created"))
        wallet_file("plain").rename(wallet_dir("w8"))
        ...
        to_create.append(str(self.options.tmpdir / 'extern/w6'))  # External, not in the wallet dir, so we need to avoid adding it to in_wallet_dir
        ...
        for wallet_name in wallet_names:
            assert_equal(wallet_file(wallet_name).is_file(), True)
        ...
        wallet_dir('w8_symlink').symlink_to(wallet_dir("w8"))
        ...
        self.nodes[0].assert_start_raises_init_error(['-walletdir=' + str(not_a_dir)], 'Error: Specified -walletdir "' + str(not_a_dir) + '" is not a directory')
        ...
        wallet_dir2.rename(wallet_dir())
        self.restart_node(0, ['-nowallet', '-walletdir=' + str(data_dir())])
        ...
        competing_wallet_dir = self.options.tmpdir / 'competing_walletdir'
        competing_wallet_dir.mkdir()
        self.restart_node(0, ['-nowallet', '-walletdir=' + str(competing_wallet_dir)])
        ...
        self.nodes[1].assert_start_raises_init_error(['-walletdir=' + str(competing_wallet_dir)], exp_stderr, match=ErrorMatch.PARTIAL_REGEX)
        ...
        wallet_dir("empty_wallet_dir").mkdir()
        ...
        new_wallet_dir = self.options.tmpdir / "new_walletdir"
        new_wallet_name = str(new_wallet_dir / "w10")
        ...
        for wallet_name in wallet_names:
            ...
            backup = self.options.tmpdir / "backup.dat"
            if backup.exists():
                backup.unlink()
        ...
        self.start_node(1)
        wallet = self.options.tmpdir / "my_wallet"
