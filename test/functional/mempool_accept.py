...
class MempoolAcceptanceTest(BitcoinTestFramework):
    ...
    def run_test(self):
        ...
        assert_raises_rpc_error(
            -8, "Array must contain between 1 and 25 transactions.", lambda: node.testmempoolaccept(rawtxs=[])
        )
        ...
        self.check_mempool_result(
            result_expected=[
                {"txid": tx.rehash(), "allowed": True, "vsize": tx.get_vsize(), "fees": {"base": fee_expected}}
            ],
        ...
        seed_tx = self.wallet.send_to(
            from_node=node, scriptPubKey=script_to_p2wsh_script(CScript([OP_TRUE])), amount=COIN
        )
        self.generate(node, 1)
        ...
        self.check_mempool_result(
            result_expected=[
                {"txid": tx.rehash(), "allowed": True, "vsize": tx.get_vsize(), "fees": {"base": Decimal("0.00001000")}}
            ],
            rawtxs=[tx.serialize().hex()],
            maxfeerate=0,
        )
