...
def calc_usage(blockdir):
    return sum(os.path.getsize(blockdir + f) for f in blockdir.iterdir() if (blockdir / f).is_file()) / (1024. * 1024.)
...
class PruneTest(BitcoinTestFramework):
    ...
    def setup_network(self):
        ...
        self.prunedir = self.nodes[2].blocks_path
    ...
    def test_height_min(self):
        ...
        self.wait_until(lambda: (self.prunedir / "blk00000.dat").is_file(), timeout=30)
    ...
    def manual_test(self, node_number, use_timestamp):
        ...
        def has_block(index):
            return (self.nodes[node_number].blocks_path / f"blk{index:05}.dat").is_file()
