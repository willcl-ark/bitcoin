...
class WalletDescriptorTest(BitcoinTestFramework):
    ...
    def test_concurrent_writes(self):
        ...
        wallet.importdescriptors(
            [
                {
                    "desc": descsum_create(
                        "wpkh(tprv8ZgxMBicQKsPeuVhWwi6wuMQGfPKi9Li5GtX35jVNknACgqe3CY4g5xgkfDDJcmtF7o1QnxWDRYw4H5P26PXq7sbcUkEqeR4fg3Kxp2tigg/0h/0h/*h)"
                    ),
                    "timestamp": "now",
                    "active": True,
                }
        ...
        with conn:
            ...
            bestblock_rec = conn.execute(
                "SELECT value FROM main WHERE hex(key) = '1262657374626C6F636B5F6E6F6D65726B6C65'"
            ).fetchone()[0]
    ...
    def run_test(self):
        ...
        assert_raises_rpc_error(
            -4, "Only legacy wallets are supported by this command", recv_wrpc.rpc.dumpwallet, "wallet.dump"
        )
        assert_raises_rpc_error(
            -4, "Only legacy wallets are supported by this command", recv_wrpc.rpc.importwallet, "wallet.dump"
        )
        ...
        assert_raises_rpc_error(
            -12, "Keypool ran out, please call keypoolrefill first", send_wrpc.getnewaddress, "", "bech32"
        )
        ...
        for addr_type, internal, desc_prefix, deriv_path, int_idx in addr_types:
            ...
            imp_rpc.importdescriptors(
                [{"desc": desc, "active": True, "next_index": 11, "timestamp": "now", "internal": internal}]
            )
        ...
        conn.close()
        assert_raises_rpc_error(
            -4, "Unexpected legacy entry in descriptor wallet found.", self.nodes[0].loadwallet, "crashme"
        )
