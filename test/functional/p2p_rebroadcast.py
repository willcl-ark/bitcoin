...
class NodeRebroadcastTest(BitcoinTestFramework):
    ...
    def make_txn_at_fee_rate(self, input_utxo, outputs, outputs_sum, desired_fee_rate, change_address):
        ...
        self.log.info(f"inputs_sum {inputs_sum}")
        ...
        self.log.info(f"current_fee_btc {current_fee_btc}")
        ...
        self.log.info(f"outputs_sum {outputs_sum}")
    ...
    def run_test(self):
        ...
        # Generate some coins
        self.generate(node, COINBASE_MATURITY + 2)
        self.wallet.rescan_utxos()

        # Split two coinbases into many small utxos
        min_relay_fee = node.getnetworkinfo()["relayfee"]
        self.log.info("Make transactions to set the cache fee rate")
        self.initial_split(self.nodes[0], min_relay_fee)
        ...
        for _ in range(len(self.wallet.get_utxos(mark_as_spent=False)) - 1000):
            self.make_txn_at_fee_rate(self.wallet.get_utxo(), outputs, outputs_sum,
                                      cache_fee_rate, change_address)
        ...
        block_txns = []
        for _ in range(600):
            block_txns.append(self.make_txn_at_fee_rate(utxos.pop(), outputs,
                                                        outputs_sum,
                                                        cache_fee_rate,
                                                        change_address))
