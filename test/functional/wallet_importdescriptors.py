...
class ImportDescriptorsTest(BitcoinTestFramework):
    def set_test_params(self):
        ...
        self.extra_args = [["-addresstype=legacy"], ["-addresstype=bech32", "-keypool=5"]]
    ...
    def run_test(self):
        ...
        self.test_importdesc({"timestamp": "now"}, success=False, error_code=-8, error_message="Descriptor not found.")
        ...
        test_address(w1, key.p2pkh_addr, solvable=True, ismine=True, labels=["Descriptor import test"])
        assert_equal(w1.getwalletinfo()["keypoolsize"], 0)
        ...
        key = get_generate_key()
        self.test_importdesc(
            {"desc": descsum_create("pkh(" + key.pubkey + ")"), "timestamp": "now", "internal": True}, success=True
        )
        ...
        self.test_importdesc(
            {"desc": descsum_create("pkh( " + key.pubkey + ")"), "timestamp": "now", "internal": True},
            error_code=-5,
        ...
        self.test_importdesc(
            {"desc": descsum_create(desc), "timestamp": "now", "range": [0, 100], "label": "test"},
            success=False,
            error_code=-8,
        ...
        self.test_importdesc(
            {"desc": descsum_create("wpkh(" + xpub + "/1h/*)"), "timestamp": "now", "range": 1},
            success=False,
            error_code=-4,
        ...
        self.test_importdesc(
            ...
            success=False,
            error_code=-4,
            error_message=f"Could not add descriptor '{range_request['desc']}': new range must include current range = [0,20]",
        )
        self.test_importdesc(
        ...
        self.test_importdesc(
            {
                "desc": descsum_create("sh(wpkh([abcdef12/0h/0h]" + xpub + "/*))"),
                "active": True,
                "range": [0, 2],
                "timestamp": "now",
            },
        ...
        assert_equal(
            addr, "bcrt1qdt0qy5p7dzhxzmegnn4ulzhard33s2809arjqgjndx87rv5vd0fq2czhy8"
        )  # Derived at m/84'/0'/0'/0
        change_addr = wmulti_priv.getrawchangeaddress("bech32")  # uses change 0
        assert_equal(
            change_addr, "bcrt1qt9uhe3a9hnq7vajl7a094z4s3crm9ttf8zw3f5v9gr2nyd7e3lnsy44n8e"
        ...
        assert_equal(
            addr, "bcrt1qp8s25ckjl7gr6x2q3dx3tn2pytwp05upkjztk6ey857tt50r5aeqn6mvr9"
        )  # Derived at m/84'/0'/0'/1
        change_addr = wmulti_pub.getrawchangeaddress("bech32")  # uses change 2
        assert_equal(
            change_addr, "bcrt1qp6j3jw8yetefte7kw6v5pc89rkgakzy98p6gf7ayslaveaxqyjusnw580c"
        ...
        self.log.info("Under P2SH, multisig are standard with up to 15 compressed keys")
        ...
        self.test_importdesc(
            {
                "desc": descsum_create(f"wpkh({xpriv}/<10;20>/0/*)"),
                "active": True,
                "range": 10,
                "timestamp": "now",
                "label": "some label",
        ...
        self.test_importdesc(
            {"desc": descsum_create(f"wpkh({xpriv}/<10;20>/0/*)"), "active": True, "range": 10, "timestamp": timestamp},
            success=True,
            wallet=w_multipath,
        )

        self.test_importdesc(
            {"desc": descsum_create(f"wpkh({xpriv}/10/0/*)"), "active": True, "range": 10, "timestamp": timestamp},
            success=True,
            wallet=w_multisplit,
        )
        self.test_importdesc(
            {
                "desc": descsum_create(f"wpkh({xpriv}/20/0/*)"),
                "active": True,
                "range": 10,
                "internal": True,
                "timestamp": timestamp,
        ...
        for _ in range(0, 10):
            assert_equal(
                w_multipath.getnewaddress(address_type="bech32"), w_multisplit.getnewaddress(address_type="bech32")
            )
            assert_equal(
                w_multipath.getrawchangeaddress(address_type="bech32"),
                w_multisplit.getrawchangeaddress(address_type="bech32"),
            )
        assert_equal(
            sorted(w_multipath.listdescriptors()["descriptors"], key=lambda x: x["desc"]),
            sorted(w_multisplit.listdescriptors()["descriptors"], key=lambda x: x["desc"]),
        )
