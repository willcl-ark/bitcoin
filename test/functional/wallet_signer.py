...
class WalletSignerTest(BitcoinTestFramework):
    ...
    def test_valid_signer(self):
        ...
        self.nodes[1].createwallet(
            wallet_name="not_hww", disable_private_keys=True, descriptors=True, external_signer=False
        )
        not_hww = self.nodes[1].get_wallet_rpc("not_hww")
        assert_equal(not_hww.getwalletinfo()["external_signer"], False)
        assert_raises_rpc_error(
            -8, "Wallet flag is immutable: external_signer", not_hww.setwalletflag, "external_signer", True
        )
        ...
        self.set_mock_result(self.nodes[1], "2")
        assert_raises_rpc_error(-1, "RunCommandParseJSON error", hww.walletdisplayaddress, address1)
        self.clear_mock_result(self.nodes[1])
        ...
        assert_raises_rpc_error(
            -1, "Signer echoed unexpected address wrong_address", hww.walletdisplayaddress, address_fail
        )
        ...
        mock_psbt_bumped_signed = mock_wallet.walletprocesspsbt(
            psbt=mock_psbt_bumped, sign=True, sighashtype="ALL", bip32derivs=True
        )
