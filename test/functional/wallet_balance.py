...
class WalletTest(BitcoinTestFramework):
    ...
    def run_test(self):
        ...
        assert_equal(
            len(self.nodes[0].listunspent(query_options={"include_immature_coinbase": False})),
            1 if self.options.descriptors else 2,
        )
        assert_equal(
            len(self.nodes[0].listunspent(query_options={"include_immature_coinbase": True})),
        ...
        assert_raises_rpc_error(
            -32, 'dummy first argument must be excluded or set to "*"', self.nodes[1].getbalance, ""
        )
        ...
        def test_balances(*, fee_node_1=0):
            ...
            assert_equal(
                self.nodes[1].getunconfirmedbalance(), Decimal("30") - fee_node_1
            )  # Doesn't include output of node 0's send since it was spent
