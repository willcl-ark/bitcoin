...
class RPCPackagesTest(BitcoinTestFramework):
    ...
    def test_independent(self, coin):
        ...
        tx_bad_sig_hex = node.createrawtransaction(
            [{"txid": coin["txid"], "vout": 0}], {address: coin["amount"] - Decimal("0.0001")}
        )
    ...
    def test_submitpackage(self):
        ...
        assert_raises_rpc_error(
            -8, f"Array must contain between 2 and {MAX_PACKAGE_COUNT} transactions.", node.submitpackage, []
        )
        assert_raises_rpc_error(
            -8,
            f"Array must contain between 2 and {MAX_PACKAGE_COUNT} transactions.",
            node.submitpackage,
            [chain_hex[0]] * 1,
        )
        assert_raises_rpc_error(
