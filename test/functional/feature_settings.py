...
class SettingsTest(BitcoinTestFramework):
    ...
    def run_test(self):
        ...
        self.stop_node(0)
        default_settings = {
            "_warning_": f"This file is automatically generated and updated by {self.config['environment']['PACKAGE_NAME']}. Please do not edit this file while the node is running, as any changes might be ignored or overwritten."
        }
        ...
        with settings.open() as fp:
            assert_equal(
                json.load(fp),
                {**default_settings, **{"string": "string", "num": 5, "bool": True, "null": None, "list": [6, 7]}},
            )
        ...
        node.assert_start_raises_init_error(
            expected_msg="does not contain valid JSON. This is probably caused by disk corruption or a crash",
        ...
        with settings.open("w") as fp:
            fp.write('"string"')
        node.assert_start_raises_init_error(
            expected_msg='Found non-object value "string" in settings file', match=ErrorMatch.PARTIAL_REGEX
        )
        ...
        node.assert_start_raises_init_error(
            expected_msg="Found duplicate key key in settings file", match=ErrorMatch.PARTIAL_REGEX
        )
