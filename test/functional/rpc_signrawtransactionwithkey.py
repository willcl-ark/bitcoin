...
class SignRawTransactionWithKeyTest(BitcoinTestFramework):
    ...
    def send_to_address(self, addr, amount):
        ...
        txid = self.nodes[0].sendrawtransaction(
            self.nodes[0].signrawtransactionwithkey(rawtx, [self.nodes[0].get_deterministic_priv_key().key])["hex"], 0
        )
        return txid
    ...
    def keyless_signing_test(self):
        ...
        spending_tx = self.nodes[0].createrawtransaction(
            [{"txid": funding_txid, "vout": 0}], [{getnewdestination()[2]: Decimal("49.998")}]
        )
    ...
    def verify_txn_with_witness_script(self, tx_type):
        ...
        # Now create and sign a transaction spending that output on node[0], which doesn't know the scripts or keys
        spending_tx = self.nodes[0].createrawtransaction(
            [{"txid": txid, "vout": vout}], {getnewdestination()[2]: Decimal("9.999")}
        )
