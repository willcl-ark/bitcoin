...
BLOCKS_VALUE_OF_ZERO = (
    "error: the first argument (number of blocks to generate, default: 1) must be an integer value greater than zero"
)
...
class TestBitcoinCli(BitcoinTestFramework):
    ...
    def run_test(self):
        ...
        assert_equal(
            self.nodes[0].cli("-named", "echo", "arg0=0", "arg1=1", "arg2=2", "arg1=3").send_cli(), ["0", "3", "2"]
        )
        assert_raises_rpc_error(
            -8,
        ...
        assert_raises_process_error(
            ...
            self.nodes[0].cli(f"-rpcuser={user}", "-stdinrpcpass", input="foo").echo,
        )

        self.log.info("Test -stdin and -stdinrpcpass")
        assert_equal(
            ["foo", "bar"],
            self.nodes[0].cli(f"-rpcuser={user}", "-stdin", "-stdinrpcpass", input=f"{password}\nfoo\nbar").echo(),
        )
        assert_raises_process_error(
            1,
            "Incorrect rpcuser or rpcpassword",
            self.nodes[0].cli(f"-rpcuser={user}", "-stdin", "-stdinrpcpass", input="foo").echo,
        )
        ...
        assert_raises_process_error(
            ...
            self.nodes[0].cli("-rpcconnect=127.0.0.1:notaport").echo,
        )
        assert_raises_process_error(
            1, "Invalid port provided in -rpcconnect: 127.0.0.1:-1", self.nodes[0].cli("-rpcconnect=127.0.0.1:-1").echo
        )
        assert_raises_process_error(
            1, "Invalid port provided in -rpcconnect: 127.0.0.1:0", self.nodes[0].cli("-rpcconnect=127.0.0.1:0").echo
        )
        assert_raises_process_error(
            1,
            "Invalid port provided in -rpcconnect: 127.0.0.1:65536",
            self.nodes[0].cli("-rpcconnect=127.0.0.1:65536").echo,
        )
        ...
        if have_ipv6:
            assert_raises_process_error(
                1,
                "Invalid port provided in -rpcconnect: [::1]:notaport",
                self.nodes[0].cli("-rpcconnect=[::1]:notaport").echo,
            )
            assert_raises_process_error(
                1, "Invalid port provided in -rpcconnect: [::1]:-1", self.nodes[0].cli("-rpcconnect=[::1]:-1").echo
            )
            assert_raises_process_error(
                1, "Invalid port provided in -rpcconnect: [::1]:0", self.nodes[0].cli("-rpcconnect=[::1]:0").echo
            )
            assert_raises_process_error(
                1,
                "Invalid port provided in -rpcconnect: [::1]:65536",
                self.nodes[0].cli("-rpcconnect=[::1]:65536").echo,
            )

        self.log.info("Test handling of invalid ports in rpcport")
        assert_raises_process_error(
            1, "Invalid port provided in -rpcport: notaport", self.nodes[0].cli("-rpcport=notaport").echo
        )
        ...
        assert_raises_process_error(
            1, "Invalid port provided in -rpcport: 65536", self.nodes[0].cli("-rpcport=65536").echo
        )
        ...
        assert_raises_process_error(
            1,
            "Could not connect to the server 127.0.0.1:1",
            self.nodes[0].cli(f"-rpcconnect=127.0.0.1:{node_rpc_port}", "-rpcport=1").echo,
        )
        if have_ipv6:
            assert_raises_process_error(
                1,
                "Could not connect to the server ::1:1",
                self.nodes[0].cli(f"-rpcconnect=[::1]:{node_rpc_port}", "-rpcport=1").echo,
            )

        assert_equal(
            BLOCKS, self.nodes[0].cli("-rpcconnect=127.0.0.1:18999", f"-rpcport={node_rpc_port}").getblockcount()
        )
        if have_ipv6:
            assert_equal(
                BLOCKS, self.nodes[0].cli("-rpcconnect=[::1]:18999", f"-rpcport={node_rpc_port}").getblockcount()
            )
        ...
        assert_equal(
            BLOCKS,
            self.nodes[0].cli("-norpccookiefile", f"-rpcuser={user}", f"-rpcpassword={password}").getblockcount(),
        )
        ...
        assert_raises_process_error(
            1, "Could not connect to the server", self.nodes[0].cli("-rpcwait", "-rpcwaittimeout=5").echo
        )
        ...
        assert_raises_process_error(
            1, "Only one of -getinfo, -netinfo may be specified", self.nodes[0].cli("-getinfo", "-netinfo").send_cli
        )
