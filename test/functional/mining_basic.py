...
class MiningTest(BitcoinTestFramework):
    ...
    def mine_chain(self):
        ...
        assert_equal(
            ...
            self.nodes[0].getblocktemplate(NORMAL_GBT_REQUEST_PARAMS)["version"],
        )
        self.restart_node(0)
        self.connect_nodes(0, 1)
    ...
    def test_fees_and_sigops(self):
        ...
        #      tx_c (3 sat/vbyte)
        #
        tx_a = wallet_sigops.send_self_transfer(from_node=node, fee_rate=Decimal("0.00001"))
        tx_b = wallet_sigops.send_self_transfer(
            from_node=node, fee_rate=Decimal("0.00002"), utxo_to_spend=tx_a["new_utxo"]
        )
        tx_c = wallet_sigops.send_self_transfer(
            from_node=node, fee_rate=Decimal("0.00003"), utxo_to_spend=tx_b["new_utxo"]
        )
        ...
        tx_d = self.wallet.send_self_transfer(from_node=node, fee_rate=Decimal("0.00100"))
        ...
        assert_equal(
            block_template_fees, [tx_d["fee"] * COIN, tx_a["fee"] * COIN, tx_b["fee"] * COIN, tx_c["fee"] * COIN]
        )
    ...
    def test_blockmintxfee_parameter(self):
        ...
        for blockmintxfee_sat_kvb in (DEFAULT_BLOCK_MIN_TX_FEE, 0, 50, 100, 500, 2500, 5000, 21000, 333333, 2500000):
            ...
            assert_equal(
                tx_with_min_feerate["fee"], get_fee(tx_with_min_feerate["tx"].get_vsize(), blockmintxfee_btc_kvb)
            )
            if blockmintxfee_btc_kvb > 0:
                ...
                assert_equal(
                    tx_below_min_feerate["fee"], get_fee(tx_below_min_feerate["tx"].get_vsize(), lowerfee_btc_kvb)
                )
    ...
    def test_timewarp(self):
        ...
        bad_block.solve()
        assert_raises_rpc_error(
            -25, "time-timewarp-attack", lambda: node.submitheader(hexdata=CBlockHeader(bad_block).serialize().hex())
        )
    ...
    def verify_block_template(self, expected_tx_count, expected_weight):
        ...
        self.log.info(
            f"Testing block template: contains {expected_tx_count} transactions, and total weight <= {expected_weight}"
        )
    ...
    def test_block_max_weight(self):
        ...
        utxos = [
            self.wallet.get_utxo(confirmed_only=True) for _ in range(LARGE_TXS_COUNT + 4)
        ]  # Add 4 more utxos that will be used in the test later
        ...
        self.log.info(
            "Testing the block template with custom -blockmaxweight to include 9 large and 2 normal transactions."
        )
        self.verify_block_template(
        ...
        self.log.info(
            f"Testing that the block weight will never exceed {MAX_BLOCK_WEIGHT - DEFAULT_BLOCK_RESERVED_WEIGHT}."
        )
        ...
        self.log.info(
            "Testing that the block template includes only 10 transactions and cannot reach full block weight."
        )
        self.verify_block_template(
            expected_tx_count=LARGE_TXS_COUNT,
            expected_weight=MAX_BLOCK_WEIGHT - DEFAULT_BLOCK_RESERVED_WEIGHT,
        ...
        self.log.info(
            f"Test that node will fail to start when user provide -blockreservedweight below {MINIMUM_BLOCK_RESERVED_WEIGHT}"
        )
        self.stop_node(0)
        self.nodes[0].assert_start_raises_init_error(
    ...
    def run_test(self):
        ...
        assert_raises_rpc_error(
            -8, "getblocktemplate must be called with the segwit rule set", node.getblocktemplate, {}
        )
        ...
        assert_raises_rpc_error(
            -22, "Block header decode failed", lambda: node.submitheader(hexdata="xx" * BLOCK_HEADER_SIZE)
        )
        assert_raises_rpc_error(
            -22, "Block header decode failed", lambda: node.submitheader(hexdata="ff" * (BLOCK_HEADER_SIZE - 2))
        )
        assert_raises_rpc_error(
            -25,
        ...
        bad_block2.solve()
        assert_raises_rpc_error(
            -25, "bad-prevblk", lambda: node.submitheader(hexdata=CBlockHeader(bad_block2).serialize().hex())
        )
        ...
        assert_raises_rpc_error(
            -25, "time-too-old", lambda: node.submitheader(hexdata=CBlockHeader(bad_block_time).serialize().hex())
        )
