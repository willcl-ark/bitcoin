...
def _get_header_file_lst() -> list[str]:
    ...
    header_file_lst = check_output(git_cmd_lst, text=True).splitlines()
...
def main():
    ...
    for header_file in header_file_lst:
        ...
        if count != 3:
            print(
                f"{header_file} seems to be missing the expected include guard to prevent the double inclusion problem:"
            )
