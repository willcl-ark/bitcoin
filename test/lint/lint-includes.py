...
def find_included_cpps():
    ...
    try:
        included_cpps = check_output(
            ["git", "grep", "-E", r"^#include [<\"][^>\"]+\.cpp[>\"]", "--", "*.cpp", "*.h"], text=True, encoding="utf8"
        ).splitlines()
...
def find_extra_boosts():
    ...
    try:
        included_boosts = check_output(
            ["git", "grep", "-E", r"^#include <boost/", "--", "*.cpp", "*.h"], text=True, encoding="utf8"
        ).splitlines()
    except CalledProcessError as e:
        if e.returncode > 1:
            raise e
...
def find_quote_syntax_inclusions():
    ...
    try:
        quote_syntax_inclusions = check_output(
            ["git", "grep", r"^#include \"", "--", "*.cpp", "*.h"] + exclude_args, text=True, encoding="utf8"
        ).splitlines()
    except CalledProcessError as e:
        if e.returncode > 1:
            raise e
...
def main():
    ...
    for expected_boost in EXPECTED_BOOST_INCLUDES:
        try:
            check_output(
                ["git", "grep", "-q", r"^#include <%s>" % expected_boost, "--", "*.cpp", "*.h"],
                text=True,
                encoding="utf8",
            )
        except CalledProcessError as e:
