...
* Update the following variables in [`src/kernel/chainparams.cpp`](/src/kernel/chainparams.cpp) for mainnet, testnet, and signet:
  ...
  - `m_assumeutxo_data` array can be appended to with the (partial) output of the `gettxoutsetinfo "hash_serialized_3 <target_block_height>"` RPC. The same height considerations as for `defaultAssumeValid` apply.
