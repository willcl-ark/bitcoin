...
Applications should treat command notifications as best-effort hints. A state
change can produce duplicate notifications or no usable notification, so
applications should make their notification handling idempotent and reconcile
state with RPCs.
...
Internal validation-interface subscribers receive queued callbacks in the order
events were generated for that subscriber, and queued callbacks for a subscriber
are invoked serially. This ordering only applies to callbacks sent through the
background validation-interface queue. Synchronous callbacks such as
`ActiveTipChange`, `BlockChecked`, and `NewPoWValidBlock` are invoked directly.
No ordering should be assumed across different subscribers.
