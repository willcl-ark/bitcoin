...
if not versions:
    print(
        f"No binaries found in {builddir}. Please ensure the binaries are present in {builddir}, or set another build path using the BUILDDIR env variable."
    )
    sys.exit(1)
...
with tempfile.NamedTemporaryFile("w", suffix=".h2m") as footer:
    ...
    for abspath, verstr, _ in versions:
        ...
        subprocess.run(
            [help2man, "-N", "--version-string=" + verstr, "--include=" + footer.name, "-o", outname, abspath],
            check=True,
        )
