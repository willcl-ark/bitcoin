...
def parse_gpg_result(output: list[str]) -> tuple[list[SigData], list[SigData], list[SigData]]:
    ...
    if all_found != total_resolved_sigs:
        raise RuntimeError(
            f"failed to evaluate all signatures: found {all_found} " f"but expected {total_resolved_sigs}"
        )
...
def get_files_from_hosts_and_compare(
    ...
    for i, host in enumerate(other_hosts):
        ...
        if require_all and not success:
            ...
            log.warning(f"{host} failed to provide file ({url}). " f"Continuing based solely upon {primary_host}.")
        else:
...
def verify_shasums_signature(
    ...
    if num_trusted < min_good_sigs:
        ...
        log.info("")
        log.error("not enough trusted sigs to meet threshold " f"({num_trusted} vs. {min_good_sigs})")
...
def verify_binary_hashes(hashes_to_verify: list[list[str]]) -> tuple[ReturnCode, dict[str, str]]:
    ...
    if offending_files:
        ...
        log.critical("Hashes don't match.\n" f"Offending files:\n{joined_files}")
