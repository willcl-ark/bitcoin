    ...
    while True:
        ...
        if not valid_sig:
            if prev_commit != "":
                ...
                parents = (
                    subprocess.check_output([GIT, "show", "-s", "--format=format:%P", prev_commit], text=True)
                    .splitlines()[0]
                    .split(" ")
                )
        ...
        if (verify_tree or prev_commit == "") and current_commit not in incorrect_sha512_allowed:
            ...
            if ("Tree-SHA512: {}".format(tree_hash)) not in subprocess.check_output(
                [GIT, "show", "-s", "--format=format:%B", current_commit], text=True
            ).splitlines():
        ...
        parents = (
            subprocess.check_output([GIT, "show", "-s", "--format=format:%P", current_commit], text=True)
            .splitlines()[0]
            .split(" ")
        )
        if len(parents) > 2:
        ...
        commit_time = int(
            subprocess.check_output([GIT, "show", "-s", "--format=format:%ct", current_commit], text=True).splitlines()[
                0
            ]
        )
        check_merge = (
        ...
        if len(parents) == 2 and check_merge and not allow_unclean:
            current_tree = subprocess.check_output(
                [GIT, "show", "--format=%T", current_commit], text=True
            ).splitlines()[0]
            ...
            try:
                recreated_tree = subprocess.check_output(
                    [GIT, "merge-tree", "--write-tree", parents[0], parents[1]], text=True
                ).splitlines()[0]
