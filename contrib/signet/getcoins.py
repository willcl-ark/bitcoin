...
class PPMImage:
    ...
    def __init__(self, f):
        ...
        self._grid = [
            [tuple(data[stride * y + 3 * x : stride * y + 3 * (x + 1)]) for x in range(width)] for y in range(height)
        ]
...
parser.add_argument(
    "-n", "--amount", dest="amount", default="0.001", help="Amount to request (0.001-0.1, default is 0.001)"
)
parser.add_argument(
    "-i", "--imagemagick", dest="imagemagick", default=CONVERT, help="Path to imagemagick convert utility"
)
...
if args.captcha != "":  # Retrieve a captcha
    ...
    try:
        rv = subprocess.run(
            [args.imagemagick, "svg:-", "-depth", "8", "ppm:-"], input=res.content, check=True, capture_output=True
        )
    except FileNotFoundError:
        raise SystemExit(
...
if res:
    ...
    print(
        "The script does not allow for repeated transactions as the global faucet is rate-limited to 1 request/IP/day. You can access the faucet website to get more coins manually"
    )
else:
