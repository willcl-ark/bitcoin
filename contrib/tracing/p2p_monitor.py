...
def main(pid):
    ...
    def handle_inbound(_, data, size):
        ...
        if event.peer_id not in peers:
            peer = Peer(event.peer_id, event.peer_addr.decode("utf-8"), event.peer_conn_type.decode("utf-8"))
            peers[peer.id] = peer
        peers[event.peer_id].add_message(Message(event.msg_type.decode("utf-8"), event.msg_size, True))
    ...
    def handle_outbound(_, data, size):
        ...
        if event.peer_id not in peers:
            peer = Peer(event.peer_id, event.peer_addr.decode("utf-8"), event.peer_conn_type.decode("utf-8"))
            peers[peer.id] = peer
        peers[event.peer_id].add_message(Message(event.msg_type.decode("utf-8"), event.msg_size, False))
...
def render(screen, peers, cur_list_pos, scroll, ROWS_AVALIABLE_FOR_LIST, info_panel):
    ...
    screen.addstr(
        3, 0, header_format % ("PEER", "OUTBOUND", "INBOUND", "TYPE", "ADDR"), curses.A_BOLD | curses.A_UNDERLINE
    )
    ...
    for i, peer_id in enumerate(peer_list):
        ...
        if i + scroll == cur_list_pos:
            ...
            info_window.erase()
            info_window.border(ord("|"), ord("|"), ord("-"), ord("-"), ord("-"), ord("-"), ord("-"), ord("-"))

            info_window.addstr(1, 1, f"PEER {peer.id} ({peer.address})".center(68), curses.A_REVERSE | curses.A_BOLD)
            info_window.addstr(2, 1, f" OUR NODE{peer.connection_type:^54}PEER ", curses.A_BOLD)
            for i, msg in enumerate(peer.last_messages):
                if msg.inbound:
                    info_window.addstr(i + 3, 1, "%68s" % (f"<--- {msg.msg_type} ({msg.size} bytes) "), curses.A_NORMAL)
                else:
                    info_window.addstr(i + 3, 1, " %s (%d byte) --->" % (msg.msg_type, msg.size), curses.A_NORMAL)
